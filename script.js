...
// Variables específicas para saber exactamente QUÉ se desbloqueó
...
function checkDeepLinks() {
    ...
    if (urlParams.has('movie')) {
        ...
        // --- VALIDACIÓN DE TICKET PARA PELÍCULAS ---
        if (parts.includes('vip')) {
            const ticketTime = localStorage.getItem('auraflix_ticket_' + id);
            const now = new Date().getTime();
            
            // Si tiene ticket y tiene menos de 30 minutos de creado
            if (ticketTime && (now - parseInt(ticketTime)) < 30 * 60 * 1000) {
                window.unlockedItemId = String(id); 
                localStorage.removeItem('auraflix_ticket_' + id); // Consumimos el ticket
            } else {
                alert("Enlace expirado o no válido. Debes pasar por el acortador.");
            }
        }
        ...
        // --- VALIDACIÓN DE TICKET PARA SERIES ---
        if (parts.includes('vip')) {
            const ticketKey = `${id}-${seasonStr}-${epStr}`;
            const ticketTime = localStorage.getItem('auraflix_ticket_' + ticketKey);
            const now = new Date().getTime();
            ...
            if (ticketTime && (now - parseInt(ticketTime)) < 30 * 60 * 1000) {
                ...
                localStorage.removeItem('auraflix_ticket_' + ticketKey); // Consumimos el ticket
            } else {
                alert("Enlace expirado o no válido. Debes pasar por el acortador.");
            }
        }
...
function openModal(id, typeHint) {
    ...
    if (isSeries) {
        ...
        // Verificamos si ESTA película específica está desbloqueada por enlace
...
function renderEpisodes(season, serieItem, autoPlayIndex = -1) {
    ...
    if (autoPlayIndex >= 0 && season.episodes[autoPlayIndex]) {
        ...
        // Comprobar si ESTE episodio en específico se desbloqueó por enlace
        const isThisEpisodeUnlocked = (window.unlockedItemId === String(serieItem.id) &&
                                       window.unlockedSeason === parseInt(season.season) &&
                                       window.unlockedEpisode === parseInt(ep.episode));
                                       
        const hasAccess = isVipGlobal || isThisEpisodeUnlocked;
    ...
    buttons.forEach((btn, index) => {
        btn.onclick = () => {
            ...
            // Re-comprobar acceso por el nuevo episodio clickeado
            const isThisEpisodeUnlocked = (window.unlockedItemId === String(serieItem.id) &&
                                           window.unlockedSeason === parseInt(season.season) &&
                                           window.unlockedEpisode === parseInt(ep.episode));
...
window.executeUnlock = function() {
    if (currentModalItem) {
        ...
        if (shortLink) {
            // GENERAMOS EL TICKET PARA VALIDAR AL REGRESAR
            localStorage.setItem('auraflix_ticket_' + ticketKey, new Date().getTime());
            window.open(shortLink, '_self');
        } else {
            alert("No hay enlace de desbloqueo configurado para este contenido aún.");
        }
    }
};
...
function setPlayerVideo(url, overlayText = null, isBlocked = false) {
    ...
    if (isBlocked) {
        ...
        if (window.UNLOCK_LINK_STATUS === 'on' || typeof window.UNLOCK_LINK_STATUS === 'undefined') {
            buttonsHtml += `<button onclick="showUnlockWarning()" class="vip-redirect-btn" style="background: linear-gradient(45deg, #6a0dad, #A66BFF); margin-bottom: 12px; width: 100%; box-shadow: 0 0 10px rgba(138, 43, 226, 0.5);"><i class="fas fa-unlock"></i> Desbloquear Contenido</button>`;
        }
...
function setupEventListeners() {
    ...
    document.getElementById('btnCache').onclick = () => {
        if(confirm("¿Borrar historial de visualización y favoritos?")) {
            ...
            continueWatching = []; myFavorites = []; seriesMemory = {};
            renderMyList();
            alert("Caché borrada.");
