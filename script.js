...
function iniciarTodo() {
    ...
    // Activar sistema de rotación automática
    setupAutoRotation();
}

// Lógica para forzar horizontal en celulares al poner pantalla completa
function setupAutoRotation() {
    ...
    document.addEventListener('mozfullscreenchange', handleRotation);
    document.addEventListener('MSFullscreenChange', handleRotation);
}
...
function openModal(id, typeHint) {
    ...
    if (isSeries) {
        ...
        if (item.seasons && item.seasons.length > 0) {
            ...
            // ASIGNAR EVENTO AL BOTÓN DE TEMPORADAS CORRECTAMENTE
            const btnSeason = document.getElementById('btnOpenSeasonModal');
            if(btnSeason) {
                btnSeason.onclick = openSeasonSelectorModal;
            }
        }
...
function openSeasonSelectorModal() {
    ...
    // Forzamos el display flex para que se vea
...
function closeModalInternal() {
    ...
    // Asegurar cierre del selector de temporadas
