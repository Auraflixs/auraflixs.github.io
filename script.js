...
function openModal(id, typeHint, fromDeepLink = false) {
    ...
    // ==========================================================
    // ANIMACIÓN HERO CORREGIDA (SE VE LA EXPANSIÓN PERFECTA)
    // ==========================================================
    const clickedCard = document.querySelector(`.item[data-id="${idStr}"] img.smooth-image`);

    if (clickedCard && !fromDeepLink) {
        // Tomamos el cuadro exacto donde está la película que tocaste
        const startRect = clickedCard.getBoundingClientRect();
        
        modal.style.display = 'flex';
        playerWrapper.style.opacity = '0'; 
        
        // CORRECCIÓN: Medimos el destino (targetRect) forzando su estado abierto temporalmente 
        // para evitar el bug de las coordenadas incorrectas ("salida de esquina")
        ...
        // Posición Inicial (idéntica a la tarjeta que tocaste)
        ...
        // FORCE REFLOW: Le decimos al celular "dibuja esto AQUÍ antes de moverlo"
        void clone.offsetWidth;

        // VELOCIDAD 650ms: Lenta, fluida y nativa
        clone.style.transition = 'all 0.65s cubic-bezier(0.25, 1, 0.5, 1)';

        // Ejecutamos la expansión hacia el destino (Player)
        requestAnimationFrame(() => {
            ...
            // Eliminar clon justo al terminar los 650ms
            setTimeout(() => {
...
// CORRECCIÓN ANIMACIÓN AL CERRAR (650ms de encogimiento suave)
function closeModalInternal() {
    ...
    setTimeout(() => {
        ...
        // Reset estilos inline para la próxima
