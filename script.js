...
function setupLatelyNew() {
    ...
    // Backward-compatible: LATELY_IDS entries can be either:
    // - "idString"
    // - { id: "idString", img: "overrideImageUrl" }
...
window.executeUnlock = function() {
    if (currentModalItem) {
        ...
        if (typeof window.vipLinksList !== 'undefined' && Array.isArray(window.vipLinksList) && window.vipLinksList.length > 0) {
            let currentIndex = parseInt(localStorage.getItem('auraflix_vip_link_index')) || 0;
            if (currentIndex >= window.vipLinksList.length) {
                currentIndex = window.vipLinksList.length - 1;
            }
            shortLink = window.vipLinksList[currentIndex];
            if (currentIndex < window.vipLinksList.length - 1) {
                localStorage.setItem('auraflix_vip_link_index', currentIndex + 1);
            }
        } else {
            if (!currentModalItem.seasons) {
        ...
        if (shortLink) {
            ...
            window.location.assign(shortLink); 
        } else {
...
function setupEventListeners() {
    ...
    // --- SHARE (perfil): abre modal y maneja compartir o copiar ---
