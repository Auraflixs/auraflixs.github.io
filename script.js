...
function checkAdminAccess() {
    ...
    if (
        href.includes('auraflixs.github.io/admin') ||
        ref.includes('auraflixs.github.io/admin') ||
        sessionStorage.getItem('auraflixAdminUnlocked') === '1'
    ) {
...
function setPlayerVideo(url, overlayText = null, isBlocked = false) {
    ...
    iframe.addEventListener('load', () => {
        if (!savedTime || !currentPlayerIframe || !currentPlayerIframe.contentWindow || window.__auraHasSeeked) return;
        const seekTo = parseFloat(savedTime);
        window.__auraHasSeeked = true;

        const trySend = (attempts) => {
            if (attempts <= 0 || !currentPlayerIframe || !currentPlayerIframe.contentWindow) return;
            currentPlayerIframe.contentWindow.postMessage({ command: 'seek', value: seekTo }, currentPlayerOrigin || '*');
            setTimeout(() => {
                if (!currentPlayerIframe || !currentPlayerIframe.contentWindow) return;
                currentPlayerIframe.contentWindow.postMessage({ command: 'play' }, currentPlayerOrigin || '*');
                setTimeout(() => trySend(attempts - 1), 1500);
            }, 300);
        };

        setTimeout(() => trySend(3), 1000);
    });
