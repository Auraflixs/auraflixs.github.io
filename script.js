            ...
            if (savedTime && currentPlayerIframe.contentWindow && !window.__auraHasSeeked) { 
                ...
                setTimeout(() => {
                    if(currentPlayerIframe && currentPlayerIframe.contentWindow) {
                        ...
                        setTimeout(() => {
                            if (currentPlayerIframe && currentPlayerIframe.contentWindow) {
                                currentPlayerIframe.contentWindow.postMessage({ command: 'seek', value: parseFloat(savedTime) }, currentPlayerOrigin);
                                setTimeout(() => {
                                    if (currentPlayerIframe && currentPlayerIframe.contentWindow) {
                                        currentPlayerIframe.contentWindow.postMessage({ command: 'play' }, currentPlayerOrigin);
                                    }
...
function setupEventListeners() {
    ...
    if(btnRequestContent) {
        btnRequestContent.onclick = () => {
            if(typeof AURA_LINKS !== 'undefined' && AURA_LINKS.request) {
                window.location.href = AURA_LINKS.request;
            } else {
