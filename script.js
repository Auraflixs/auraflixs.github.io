...
// AQUÍ AGREGUÉ EL DATA-ID PARA PODER CREAR LA ANIMACIÓN 3D
...
function openModal(id, typeHint, fromDeepLink = false) {
    ...
    // ==========================================================
    const clickedCard = document.querySelector(`.item[data-id="${idStr}"] img`);

    if (clickedCard && !fromDeepLink) {
        const startRect = clickedCard.getBoundingClientRect();
        
        modal.style.display = 'flex';
        playerWrapper.style.opacity = '0'; // Ocultamos el reproductor real inicialmente
        ...
        requestAnimationFrame(() => {
            ...
            // Activamos la expansión de la imagen y el fondo oscuro
            requestAnimationFrame(() => {
                clone.style.transform = 'translate(0, 0) scale(1, 1)';
                clone.style.borderRadius = '0px';
                modal.classList.add('active'); // Fondo a negro

                // Slide Up de la información
                setTimeout(() => {
                    modal.classList.add('open');
                }, 100);

                // Limpieza al terminar transición (aprox 400ms)
                setTimeout(() => {
        ...
        // Apertura estándar sin animación Hero (ej. recargar página)
        modal.style.display = 'flex';
        playerWrapper.style.opacity = '1';
...
function closeModalInternal() {
    ...
    // Desvanecemos el modal y encogemos el contenido suavemente
    modal.classList.remove('active', 'open');
