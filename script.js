...
function initApp() {
    // IMPORTANTE: Barajamos SOLO si no se ha hecho antes, 
    // pero para la lista ordenada usaremos el orden original de carga.
    if (!window.moviesList.shuffled) {
        // Hacemos una copia para el shuffle interno si quieres aleatoriedad en "Home", 
        // pero mantenemos el orden original para "Recién Agregados"
    ...
    // AQUÍ ESTÁ EL CAMBIO PARA EL ORDEN
    let orderedContent = [];
    
    // Usamos .originalOrder si existe para respetar tu lista de archivos, 
    // si no, usamos la lista normal.
    ...
    if (window.globalContent && window.globalContent.length > 0) {
        ...
        orderedContent = [...sourceMovies, ...sourceSeries];
    }
    
    // CAMBIO: Eliminado .reverse(). Ahora toma los primeros elementos (los de arriba en tu lista).
    renderList('newlyAddedRow', orderedContent.slice(0, 20));
...
function setupHero() {
    // Usamos el orden original si existe para buscar IDs correctamente
    const sourceMovies = window.moviesList.originalOrder || window.moviesList;
    const sourceSeries = window.seriesList.originalOrder || window.seriesList;
...
function setupLatelyNew() {
    ...
    // Pasamos index para priorizar las primeras
    container.innerHTML = displayList.map((item, index) => createItemHTML(item, index)).join('');
}

function renderHomeView() {
    // Usamos la lista mezclada (shuffled) para la variedad en Home
...
function createItemHTML(item, index = 100) {
    ...
    // Priorizamos carga si está muy arriba (index bajo)
...
function renderFilteredMovies() {
    ...
    // Usamos el orden original al filtrar si el usuario quiere buscar
    const sourceMovies = window.moviesList.originalOrder || window.moviesList;
    let listToShow = [...sourceMovies];
