...
function initApp() {
    ...
    // --- LÓGICA DE RECIÉN AGREGADAS (ESTRICTO) ---
    // Usamos window.allContentSequence que guarda el orden exacto de carga de los scripts.
    // Al usar .reverse(), el último script cargado (ej: monstruoserie.js) queda de primero.
...
// --- HERO (ESTÁTICO SEGÚN ID.JS) ---
function setupHero() {
    const allContent = [...moviesListInternal, ...seriesListInternal];
    featuredList = [];

    // Prioridad 1: IDs definidos en id.js
    ...
    // Prioridad 2: Si falla id.js, primeras 5 con 'featured' (SIN MEZCLAR)
