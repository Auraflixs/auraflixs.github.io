...
// Estado de filtros (separado para pelis y series)
let movieFilters = [];
let seriesFilters = [];
let tempFilters = []; // Lo que seleccionas en el modal antes de dar "Aplicar"
let currentFilterContext = ''; // 'movies' o 'series'
...
function initApp() {
    ...
    // Orden de llegada para "Recién Agregadas"
    let orderedContent = [];
    ...
    // Mostrar las últimas 20 agregadas (reverse para que la última salga primero)
...
function setupHero() {
    ...
    // Buscar IDs específicos si existen
    ...
    // Fallback: usar propiedad featured o los primeros 5
...
function openFilterModal(context) {
    ...
    // Copiar filtros actuales a temporales para editar
    if (context === 'movies') {
...
function applyFilters() {
    // Guardar los filtros temporales en la variable real
...
// Helper para normalizar géneros (puede venir como string "Action, Drama" o array ["Action"])
...
function renderFilteredMovies() {
    ...
    if (movieFilters.length > 0) {
        ...
        listToShow = listToShow.filter(item => {
            ...
            // Mostrar si tiene AL MENOS UNO de los géneros seleccionados
            return movieFilters.some(filter => itemGenres.includes(filter));
        });
    } else {
        displayFilterText.style.display = 'none';
        shuffleArray(listToShow); // Si no hay filtros, mostramos aleatorio
    }
    
    if (listToShow.length === 0) {
        container.innerHTML = '<p style="padding:20px; color:#666; width:100%; text-align:center;">No hay resultados con estos géneros.</p>';
    } else {
...
function renderFilteredSeries() {
    ...
    if (seriesFilters.length > 0) {
        ...
        listToShow = listToShow.filter(item => {
            const itemGenres = getItemGenres(item);
            return seriesFilters.some(filter => itemGenres.includes(filter));
        });
    } else {
        displayFilterText.style.display = 'none';
...
function openModal(id, typeHint) {
    ...
    // Mostrar géneros en el modal
    let genreText = "";
    const g = getItemGenres(item);
    if (g.length > 0) {
        genreText = g.slice(0, 3).join(' / '); // Mostrar máx 3
    }
    ...
    // Configuración Específica Serie vs Peli
    if (isSeries) {
...
function switchView(viewName, pushToHistory = true) {
    ...
    if (viewName === 'home') {
        ...
        // Renderizar usando filtros si existen
        renderFilteredMovies();
...
window.addEventListener('popstate', (event) => {
    ...
    if (!event.state || !event.state.modal) {
        ...
        document.getElementById('filterModal').style.display = 'none'; // Cerrar filtro si se da atrás
    }
    // Cerrar overlays
