...
// NUEVO: ANIMACIÓN DE CARGA (ESQUELETO)
        ...
        if (parts.includes('vip')) {
            ...
            if (!ref.includes('linkvertise.com')) {
                ...
                if (ticketTime && (now - parseInt(ticketTime)) < 1800000) {
                    ...
                    alert("Error V4 (Sesión expirada)");
                    window.history.replaceState({}, '', window.location.href.replace('-vip', ''));
                }
            }
        }
...
// ANIMACIÓN DE CARGA PARA LAS IMÁGENES DEL HERO (EAGER)
function renderHero() {
...
// NUEVO: ANIMACIÓN "SKELETON" (CARGADOR) EN LAS PORTADAS MIENTRAS SE DESCARGAN CON LAZY
function createItemHTML(item, index = 100) {
    const type = item.seasons ? 'series' : 'movies';
    const safeId = String(item.id).replace(/'/g, "\\'");
    // Las primeras se cargan rápido, el resto usa Lazy Load para ahorrar ancho de banda
