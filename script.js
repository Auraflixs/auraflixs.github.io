    ...
    if (urlParams.has('movie')) {
        ...
        if (parts.includes('vip')) {
            ...
            if (ticketTime && (now - parseInt(ticketTime)) < 1800000) {
                ...
                // Limpiar el '-vip' de la URL para que si recarga no confunda al sistema
                window.history.replaceState({}, '', window.location.href.replace('-vip', ''));
            } else if (!unlockedMemory.includes(String(id))) {
                // Si no tiene ticket y no está en la memoria permanente, es un enlace copiado ilegalmente
...
function openModal(id, typeHint) {
    ...
    if (isSeries) {
        ...
        // Verificamos si la película está en la memoria de desbloqueados O si se acaba de desbloquear por enlace
...
function renderEpisodes(season, serieItem, autoPlayIndex = -1) {
    ...
    // Función auxiliar para chequear acceso del episodio
...
function closeModalInternal() {
    ...
    // NO limpiamos la memoria temporal aquí, para que recargar no rompa el video que estás viendo
