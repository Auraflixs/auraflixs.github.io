...
// ==========================================
// SEGURIDAD URL ARREGLADA (Elimina -vip siempre por seguridad)
// ==========================================
...
// CONTIENE EL DATA-ID CLAVE PARA LA ANIMACIÓN HERO
...
function openModal(id, typeHint, fromDeepLink = false) {
    ...
    const playerWrapper = document.getElementById('modalContentPlayer');
    const modalContent = document.querySelector('.modal-content');

    document.body.classList.add('modal-open');
    modalContent.scrollTop = 0;
    
    // Limpiamos clases de animación pasadas
    ...
    if (isSeries) {
        ...
        hasAccess = isVipGlobal || isUnlockedViaLink || isUnlockedViaMemory;
        
        // CORRECCIÓN: Botón compartir e historial SOLO si está desbloqueado
    ...
    // ==========================================================
    // INICIO DE LA ANIMACIÓN HERO ARREGLADA (FLIP FLUIDO)
    // ==========================================================
    const clickedCard = document.querySelector(`.item[data-id="${idStr}"] img.smooth-image`);

    if (clickedCard && !fromDeepLink) {
        // 1. Tomar posición inicial de la tarjeta
        const startRect = clickedCard.getBoundingClientRect();
        
        // Preparamos el modal (visible pero sin fondo negro aún)
        modal.style.display = 'flex';
        playerWrapper.style.opacity = '0'; // Ocultamos el reproductor real
        
        // Obtenemos el tamaño real del reproductor en pantalla
        const targetRect = playerWrapper.getBoundingClientRect();

        // 2. Creamos el clon exacto de la imagen
        const clone = clickedCard.cloneNode();
        clone.classList.add('hero-clone');
        
        // Lo posicionamos EXACTAMENTE sobre la tarjeta original en tamaño real (sin transforms raros)
        ...
        // 3. FORCE REFLOW: Obligamos al navegador a "dibujar" el clon en la tarjeta original antes de moverlo
        void clone.offsetWidth;

        // 4. Aplicamos la transición suave
        clone.style.transition = 'all 0.45s cubic-bezier(0.25, 1, 0.5, 1)';

        // 5. Expandimos el clon hacia el tamaño/posición del reproductor
        ...
        // Animamos el fondo a negro oscuro
        modal.classList.add('active'); 

        // Slide Up de la información con un ligerísimo retraso para verse elegante
        setTimeout(() => {
            modal.classList.add('open');
        }, 80);

        // Limpieza cuando termine la animación (450ms)
        setTimeout(() => {
        ...
        // Apertura estándar sin animación si no tocaste la tarjeta
        modal.style.display = 'flex';
        playerWrapper.style.opacity = '1';
        requestAnimationFrame(() => {
...
// CORRECCIÓN ANIMACIÓN: Encogimiento suave al cerrar el modal
function closeModalInternal() {
    const modal = document.getElementById('videoModal');
    const modalContent = document.querySelector('.modal-content');

    // Desvanecemos el modal y encogemos el contenido suavemente hacia abajo
    modal.classList.remove('active', 'open');
    modalContent.style.transition = 'transform 0.3s cubic-bezier(0.25, 1, 0.5, 1)';
    modalContent.style.transform = 'scale(0.96) translateY(20px)';
    ...
    setTimeout(() => {
        ...
        modalContent.style.transform = 'none'; // Reseteamos la posición
...
function setupEventListeners() {
    ...
    // CORRECCIÓN BUSCADOR: Ocultar sugerencias populares al escribir
