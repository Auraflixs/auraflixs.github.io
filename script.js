...
// --- 1) BLOQUE AÑADIDO: MEMORIA DE TIEMPO DEL REPRODUCTOR ---
...
window.__auraHasSeeked = false; // Fix de pantalla negra
...
function getStableVideoId(url) { 
    try { 
        ...
        // Prioridad 1: ID del hash, por ejemplo #xt9o6 
        if (u.hash && u.hash.length > 1) { 
            return decodeURIComponent( u.hash.replace(/^#/, '').split('&')[0].split('?')[0] ); 
        } 
        // Prioridad 2: parámetro filename 
        const filename = u.searchParams.get('filename'); 
        if (filename) { 
            return decodeURIComponent(filename); 
        } 
        // Prioridad 3: último segmento de la ruta 
    ...
    window.addEventListener('message', (e) => { 
        ...
        if (isReady && currentPlayerProgressKey) { 
            ...
            if (savedTime && currentPlayerIframe.contentWindow && !window.__auraHasSeeked) { 
                ...
                // Retraso para que el reproductor inicie su buffer y comando extra de PLAY
                setTimeout(() => {
                    if(currentPlayerIframe && currentPlayerIframe.contentWindow) {
                        ...
                        // Asegurar el play por si el seek lo pausa o queda en negro
                        setTimeout(() => {
                            if (currentPlayerIframe && currentPlayerIframe.contentWindow) {
...
function initLogoTyping() {
    ...
    function type() {
        ...
        if (!isDeleting && charIndex < textToType.length) {
            charIndex++;
            setTimeout(type, 150); // typing delay
        } else if (isDeleting && charIndex > 0) {
            charIndex--;
            setTimeout(type, 100); // erasing delay
        } else {
            isDeleting = !isDeleting;
            setTimeout(type, isDeleting ? 3000 : 500); // wait before reverse
        }
    }
...
function iniciarTodo() {
    initLogoTyping(); // Iniciar animación reparada del logo
    injectImageStyles();
    optimizeConnections();
...
function checkDeepLinks() {
    ...
    if (urlParams.has('movie') || urlParams.has('serie')) {
        ...
        // Inyectar el estado del inicio para que el botón volver regrese al menú principal
...
function openModal(id, typeHint, fromDeepLink = false) {
    ...
    // Manejo de URL en Historial
    if (!fromDeepLink) {
        let cleanUrl = `${window.location.origin}${window.location.pathname}`;
        if (!isSeries) {
            cleanUrl += `?movie=${item.id}-${item.year || '2025'}`;
        } else {
            // Se agregará el link con temporada y episodio en renderEpisodes
        }
...
// --- 2) BLOQUE DE REEMPLAZO COMPLETO PARA SETPLAYERVIDEO ---
function setPlayerVideo(url, overlayText = null, isBlocked = false) { 
    const playerDiv = document.getElementById('modalContentPlayer'); 
    playerDiv.innerHTML = ''; 
    window.__auraHasSeeked = false; // Resetear la memoria de salto en nuevo vídeo
...
function closeModalInternal() {
    ...
    // --- 3) BLOQUE AÑADIDO: LIMPIEZA DE MEMORIA AL CERRAR ---
    ...
    // ELIMINADA manipulación extraña del historial. Popstate ahora maneja esto perfectamente.
}
...
function setupEventListeners() {
    ...
    // FIX CERRAR Y LINKS DIRECTOS: delegamos el cierre enteramente a window.history.back()
...
// FIX MANEJO CORRECTO DEL CIERRE POR HISTORIAL (Cerrar / Botón físico del celular)
