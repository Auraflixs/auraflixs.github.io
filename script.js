...
function renderHero() {
    ...
    // HERO: fetchpriority="high" y loading="eager" para máxima velocidad y prioridad
...
function renderMultiRow(containerId, list) {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        // Pasamos index para dar prioridad
        rowDiv.innerHTML = chunk.map((item, j) => createItemHTML(item, j)).join('');
        container.appendChild(rowDiv);
    }
}

// RESTAURADO A MÉTODO NATIVO (Sin data-src)
// Usa loading="lazy" por defecto, lo que garantiza el orden de carga (arriba -> abajo)
// Usa fetchpriority="low" para que no compitan con el Hero
function createItemHTML(item, index = 100) {
    const type = item.seasons ? 'series' : 'movies';
    const safeId = String(item.id).replace(/'/g, "\\'");
    
    // Si el índice es bajo (está arriba), quizás queramos cargar un poco antes,
    // pero generalmente 'lazy' es lo mejor para listas largas.
