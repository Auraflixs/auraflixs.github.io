...
window.__auraSeekRetries = 0;
window.__auraMaxSeekRetries = 3;
let playerReadyTimeout = null;
let playerLoadAttempts = 0;
let maxPlayerLoadAttempts = 3;

let freeMinuteStartTime = null;
let freeMinuteActive = false;
let freeMinuteTimer = null;
let playerFirstPlayDetected = false;
...
function shouldApplyFreeMinuteBlock() {
    if (window.isAdminMode) return false;
    if (typeof isUserVip === 'function' && isUserVip()) return false;
    if (window.VIP_SYSTEM_STATUS === 'off' && window.UNLOCK_LINK_STATUS === 'off') return false;
    if (!currentModalItem) return false;
    
    if (!currentModalItem.seasons) {
        const isUnlockedViaMemory = isTicketCurrentlyUnlocked(String(currentModalItem.id));
        const isUnlockedViaLink = (window.unlockedItemId === String(currentModalItem.id));
        if (isUnlockedViaMemory || isUnlockedViaLink) return false;
    } else {
        ...
        const ticketKey = `${currentModalItem.id}-t${seasonObj.season}-e${epNumber}`;
        const isUnlockedViaMemory = isTicketCurrentlyUnlocked(ticketKey);
        const justUnlocked = (window.unlockedItemId === String(currentModalItem.id) && 
                             window.unlockedSeason === parseInt(seasonObj.season) && 
                             window.unlockedEpisode === parseInt(epNumber));
        if (isUnlockedViaMemory || justUnlocked) return false;
    }
    
    return true;
}

function startFreeMinute() {
    if (!shouldApplyFreeMinuteBlock()) {
        return;
    }
    
    if (freeMinuteActive) return;
    
    freeMinuteStartTime = Date.now();
    freeMinuteActive = true;
    
    if (freeMinuteTimer) {
        clearTimeout(freeMinuteTimer);
    }
    
    freeMinuteTimer = setTimeout(() => {
        applyBlockAfterFreeMinute();
    }, 60000);
}

function applyBlockAfterFreeMinute() {
    if (!freeMinuteActive) return;
    
    freeMinuteActive = false;
    ...
    const existingOverlay = playerDiv.querySelector('.vip-player-overlay');
    if (existingOverlay) return;
    
    const container = playerDiv.querySelector('.video-container');
    if (!container) return;
    ...
    blockerDiv.innerHTML = '<i class="fas fa-lock vip-lock-icon"></i><p class="vip-overlay-text" style="margin-bottom: 25px;">Minuto Gratuito Finalizado</p><div style="display: flex; flex-direction: column; width: 85%; max-width: 250px;">' + buttonsHtml + '</div>';
    
    container.appendChild(blockerDiv);
    
    showToast("Minuto gratuito finalizado. Desbloquea para continuar.");
}

function checkOneMinuteBlock(currentTime) {
    if (!freeMinuteActive) return;
    
    const elapsed = Date.now() - freeMinuteStartTime;
    if (elapsed >= 60000) {
        applyBlockAfterFreeMinute();
    }
}

function resetFreeMinute() {
    if (freeMinuteTimer) {
        clearTimeout(freeMinuteTimer);
        freeMinuteTimer = null;
    }
    freeMinuteStartTime = null;
    freeMinuteActive = false;
    playerFirstPlayDetected = false;
}
...
function installPlayerMessageHandler() {
    ...
    window.addEventListener('message', (e) => {
        ...
        const isReady = data.playerStatus === 'Ready' || 
        ...
        const isPlaying = data.event === 'playing' || data.playerStatus === 'Playing';
        ...
        if (isReady && currentPlayerProgressKey) {
            ...
            if (savedTime && currentPlayerIframe.contentWindow && !window.__auraHasSeeked) {
        ...
        if (isPlaying && !playerFirstPlayDetected) {
            playerFirstPlayDetected = true;
            if (shouldApplyFreeMinuteBlock()) {
                startFreeMinute();
            }
        }
        ...
        if (typeof currentTime === 'number' && currentPlayerProgressKey) {
            if (window.__auraHasSeeked && window.__auraSeekRetries < window.__auraMaxSeekRetries) {
                const savedTime = localStorage.getItem(currentPlayerProgressKey);
                if (savedTime) {
                    const targetTime = parseFloat(savedTime);
                    if (Math.abs(currentTime - targetTime) > 5) {
                        window.__auraSeekRetries++;
                        if (currentPlayerIframe && currentPlayerIframe.contentWindow) {
                            currentPlayerIframe.contentWindow.postMessage(
                                { command: 'seek', value: targetTime },
                                currentPlayerOrigin
            ...
            checkOneMinuteBlock(currentTime);
        }
    });
}
...
function checkDeepLinks() {
    ...
    if (urlParams.has('page') && urlParams.get('page') === 'solicitudes') {
        const baseUrl = window.location.origin + window.location.pathname;
        window.history.replaceState({ view: 'home', modal: false, search: false, history: false }, '', baseUrl);
        setTimeout(() => {
...
function openModal(id, typeHint, fromDeepLink = false) {
    ...
    const isVipGlobal = window.isAdminMode || (typeof isUserVip === 'function' ? isUserVip() : false);
    ...
    if (isSeries) {
        ...
        setPlayerVideo(item.video, null, false);
...
function renderEpisodes(season, serieItem, autoPlayIndex = -1) {
    ...
    if (autoPlayIndex >= 0 && season.episodes[autoPlayIndex]) {
        ...
        setPlayerVideo(ep.video, `T${season.season}: Cap ${ep.episode}`, false);
...
function setPlayerVideo(url, overlayText = null, isBlocked = false) {
    ...
    iframe.onload = () => {
        setTimeout(() => {
            if (currentPlayerOrigin && currentPlayerIframe && currentPlayerIframe.contentWindow) {
                try {
                    currentPlayerIframe.contentWindow.postMessage(
                        { command: 'play' },
                        currentPlayerOrigin
    ...
    playerReadyTimeout = setTimeout(() => {
        if (playerLoadAttempts < maxPlayerLoadAttempts) {
            playerLoadAttempts++;
            if (currentPlayerIframe) {
                const retryUrl = currentPlayerIframe.src;
                const retryOverlay = overlayText;
                const retryBlocked = isBlocked;
                setPlayerVideo(retryUrl, retryOverlay, retryBlocked);
            }
        }
    }, 7000);
