...
// Determinar si usar grid-layout (para peliculas/series) o horizontal-scroll (para inicio)
const isGrid = (containerId === 'movieCategoriesContainer' || containerId === 'seriesCategoriesContainer');
const rowClass = isGrid ? 'row grid-layout' : 'row horizontal-scroll';
...
categoriesData.forEach(category => {  
    ...
    if (categoryItems.length > 0) {  
        ...
        // Añadir línea divisoria para separar visualmente
        if (isGrid) {
            htmlOutput += `<hr style="border: 0; height: 1px; background: #333; margin: 15px 4% 20px 4%;">`;
        }
    }  
});  
