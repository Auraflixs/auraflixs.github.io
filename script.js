...
function openModal(id, typeHint) {
    ...
    if (shareBtn) {
        shareBtn.onclick = () => {
            ...
            // EL LINK SE COMPARTE NORMAL SIN "VIP"
            if (!isSeries) {
    ...
    const hasAccess = isVipGlobal || window.sessionUnlockedVIP;
...
// BOTÓN: DESBLOQUEAR -> Abre Acortador
window.unlockViaLink = function() {
    if (currentModalItem) {
        let shortLink = null;
        
        if (!currentModalItem.seasons && typeof window.vippLinks !== 'undefined') {
            const linkData = window.vippLinks.find(l => String(l.id) === String(currentModalItem.id));
            if (linkData) shortLink = linkData.link;
        } 
        else if (currentModalItem.seasons && typeof window.vipsLinks !== 'undefined') {
            ...
            const linkData = window.vipsLinks.find(l => 
                String(l.id) === String(currentModalItem.id) && 
                parseInt(l.season) === parseInt(seasonObj.season) && 
                parseInt(l.episode) === parseInt(epNumber)
            );
            if (linkData) shortLink = linkData.link;
        }
...
function setPlayerVideo(url, overlayText = null, isBlocked = false) {
    ...
    // CREACIÓN DE LOS DOS BOTONES DINÁMICOS
    if (isBlocked) {
        ...
        if (window.UNLOCK_LINK_STATUS === 'on') {
            buttonsHtml += `<button onclick="unlockViaLink()" class="vip-redirect-btn" style="background: linear-gradient(45deg, #ff4444, #ff6b6b); margin-bottom: 12px; width: 100%;"><i class="fas fa-unlock"></i> Desbloquear Contenido</button>`;
        }
