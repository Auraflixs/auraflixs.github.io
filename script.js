...
function iniciarTodo() {
    // ============================================================
    // SOLUCIÓN DEFINITIVA AL PROBLEMA DE CARGA (RACE CONDITION)
    // ============================================================
    // Verificamos si 'moviesList' tiene datos O si 'profileImages' existe.
    // Si no existen, esperamos 50ms y volvemos a intentar (loop de espera).
    const moviesReady = window.moviesList && window.moviesList.length > 0;
    const profilesReady = typeof profileImages !== 'undefined' && profileImages.length > 0;
    
    // Si no hay datos y han pasado menos de 5 segundos desde la carga (evita bucle infinito si la BD está vacía)
    if ((!moviesReady || !profilesReady) && performance.now() < 5000) {
        setTimeout(iniciarTodo, 50); 
        return; 
    }
    // ============================================================

    // Asegurar arrays si falló todo
...
function initApp() {
    // Mezclar listas solo si no se ha hecho
    ...
    // Lista unificada para 'Recién agregadas'
    let orderedContent = [];
...
function loadUserDataInUI() {
    ...
    // Actualizar UI VIP si existe la función
    if (typeof checkVipTimerUI === 'function') {
        checkVipTimerUI();
    }
...
// MOSTRAR PANEL ADMIN SOLO A USUARIOS ESPECÍFICOS
...
// ==========================================
// SISTEMA DE DIAGNÓSTICO DE ENLACES
// ==========================================
...
async function startDiagnosis(mode) {
    let confirmMsg = "";
    if (mode === 'content') confirmMsg = "¿Verificar todas las Películas y Capítulos? Esto puede tardar.";
    else confirmMsg = "¿Verificar todos los Tráilers? Esto será rápido.";
    ...
    modal.style.display = 'flex';
    title.innerText = mode === 'content' ? "Verificando Contenido" : "Verificando Tráilers";
    log.innerHTML = '';
    bar.style.width = '0%';
    ...
    for (let i = 0; i < total; i++) {
        ...
        status.innerText = `Analizando ${i + 1} de ${total}...`;
        bar.style.width = `${percent}%`;

        await checkLink(item, log);
    }
...
async function checkLink(item, logContainer) {
    ...
    try {
        ...
        if (error.name === 'AbortError') {
            logToConsole(logContainer, item.title, "Lento / Timeout", "warning");
        } else {
...
async function checkVimeoOfficial(title, videoUrl, container) {
    try {
        ...
        if (!videoId) {
            logToConsole(container, title, "Vimeo Inválido", "warning");
            return;
        }
        ...
        if (response.status === 404) {
            logToConsole(container, title, "Eliminado de Vimeo", "error");
        } 
    } catch (e) {
