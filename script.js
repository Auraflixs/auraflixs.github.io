...
function checkDeepLinks() {
    ...
    if (urlParams.has('movie')) {
        ...
        window.unlockedItemId = id; // Guardamos el ID que se acaba de desbloquear por enlace
...
function openModal(id, typeHint, fromDeepLink = false) {
    ...
    if (isSeries) {
        ...
        // --- LÓGICA ESTRICTA PARA PELÍCULAS ---
        const movieHasLink = item.unlockLink && item.unlockLink.trim() !== '';
        const shortenerActive = (window.UNLOCK_LINK_STATUS === 'on');
        
        // El usuario tiene acceso SI: Es Admin, es VIP, ya lo desbloqueó, el acortador está apagado GLOBALMENTE, o la película NO requiere desbloqueo.
        const hasAccess = isGlobalAdmin || isVipGlobal || 
                          unlockedMemory.includes(String(item.id)) || 
                          (window.unlockedItemId === String(item.id)) ||
                          (!shortenerActive) || (!movieHasLink);
...
function renderEpisodes(season, serieItem, autoPlayIndex = -1) {
    ...
    // --- LÓGICA ESTRICTA PARA SERIES ---
    const checkAccess = (ep) => {
        const isVipGlobal = typeof isUserVip === 'function' ? isUserVip() : false;
        const isGlobalAdmin = window.isUserAdmin ? window.isUserAdmin() : false;
        const epHasLink = ep.unlockLink && ep.unlockLink.trim() !== '';
        const shortenerActive = (window.UNLOCK_LINK_STATUS === 'on');

        if (isGlobalAdmin || isVipGlobal) return true;
        if (!shortenerActive || !epHasLink) return true;
...
function setPlayerVideo(url, overlayText = null, isBlocked = false) {
    ...
    if (isBlocked) {
        ...
        if (window.UNLOCK_LINK_STATUS === 'on') {
