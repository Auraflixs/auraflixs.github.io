...
// ==========================================
// SISTEMA DE NOTIFICACIONES (NEGRO TRANSPARENTE)
// ==========================================
function showToast(message) {
    ...
    toast.style.bottom = '105px'; // Sube un poco para mostrarse
    
    clearTimeout(toast.timer);
    toast.timer = setTimeout(() => {
        toast.style.opacity = '0';
        toast.style.bottom = '80px'; // Baja suavemente al ocultarse
    }, 3500); 
}
...
function checkDeepLinks() {
    ...
    if (urlParams.has('movie')) {
        ...
        if (parts.includes('vip')) {
            // Sin validación por URL de origen para que el APK funcione al 100%
...
function openModal(id, typeHint, fromDeepLink = false) {
    ...
    if (isSeries) {
        ...
        // SOLO GUARDAR EN HISTORIAL SI ESTÁ DESBLOQUEADA O ES GRATIS
        if (hasAccess) {
...
function renderEpisodes(season, serieItem, autoPlayIndex = -1) {
    ...
    const checkAccess = (ep) => {
        // VERIFICAMOS SI EL CAPÍTULO ES GRATIS O NECESITA DESBLOQUEO
    ...
    if (autoPlayIndex >= 0 && season.episodes[autoPlayIndex]) {
        ...
        // SOLO GUARDAR EN HISTORIAL SI EL CAPÍTULO ESTÁ DESBLOQUEADO O ES GRATIS
        if (hasAccess) {
...
function showCustomConfirm(title, message, onAcceptCallback) {
    ...
    // Clonamos los botones para borrar eventos anteriores
...
function setupEventListeners() {
    ...
    // Se eliminó la actualización de la app (btnUpdateApp, etc)
    ...
    // APLICAMOS LA CONFIRMACIÓN CON ESTILO OSCURO
    document.getElementById('btnCache').onclick = () => {
        document.getElementById('settingsModal').style.display = 'none'; // Cerramos el modal de ajustes
...
window.addEventListener('popstate', (event) => {
    ...
    if (!event.state || !event.state.modal) {
        ...
        document.getElementById('requestModal').style.display = 'none';
        document.getElementById('reportModal').style.display = 'none';
        document.getElementById('confirmModal').style.display = 'none'; // También se oculta con retroceso
