...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Listener para subir imagen desde galería
    document.getElementById('customAvatarInput').addEventListener('change', handleImageUpload);
});

// --- LÓGICA DE LOGIN / PERFIL Y SUBIDA DE IMAGEN ---
function checkLoginStatus() {
    if (!currentUser) {
        document.getElementById('loginScreen').style.display = 'flex';
        renderAvatarSelection('avatarGrid');
    } else {
        loadUserDataInUI();
        initApp();
    }
...
// Renderiza la cuadrícula de avatares + el botón de subida
...
function handleImageUpload(e) {
    ...
    reader.onload = function(event) {
        selectedAvatarTemp = event.target.result; // URL Base64
        
        // Marcamos visualmente que se seleccionó algo (podríamos mostrar preview)
        alert("Imagen cargada. Seleccionada automáticamente.");
        
        // Si estamos en el modal de cambiar avatar, guardamos directamente
        if(document.getElementById('changeAvatarModal').style.display === 'flex') {
...
document.getElementById('profilePageImg').addEventListener('click', () => {
    ...
    renderAvatarSelection('changeAvatarGrid');
});

// Cerrar modal de cambio de avatar
...
function loadUserDataInUI() {
    ...
    renderMyList(); // Renderizar favoritos
}

document.getElementById('logoutBtn').addEventListener('click', () => {
    if(confirm("¿Seguro que quieres borrar tu perfil?")) {
        localStorage.removeItem('auraflixUser');
        localStorage.removeItem('myFavorites'); // Opcional: Borrar favoritos también
        location.reload();
    }
});
...
function toggleFavorite() {
    ...
    if (index === -1) {
        // Agregar
        myFavorites.push(currentModalItem);
...
function switchView(viewName, pushToHistory = true) {
    ...
    if (viewName === 'home') {
        ...
        renderMyList(); // Refrescar lista al entrar
    }
...
function openModal(id, type) {
    ...
    // Verificar si es favorito para pintar el icono
