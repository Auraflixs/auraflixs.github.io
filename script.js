...
function showToast(message) {
    ...
    toast.style.bottom = '95px'; // Sube un poco al aparecer
    
    clearTimeout(toast.timer);
    toast.timer = setTimeout(() => {
        toast.style.opacity = '0';
        toast.style.bottom = '80px';
    }, 3500); // Desaparece a los 3.5 segundos
}

// BUSCADOR MEJORADO: Ignora signos de puntuación como : , . -
...
function iniciarTodo() {
    ...
    // Inicializa la burbuja líquida de la barra de navegación
    setTimeout(updateNavIndicator, 200);
    window.addEventListener('resize', updateNavIndicator);
}

// ==========================================
// LÓGICA LIQUID GLASS INDICATOR
// ==========================================
function updateNavIndicator() {
    ...
    if (activeItem && indicator) {
        ...
        // Ajustamos el ancho (65% del ancho del botón para hacer la pastilla perfecta)
        const bubbleWidth = itemRect.width * 0.65; 
        indicator.style.width = `${bubbleWidth}px`; 
        
        // Calculamos la posición izquierda para centrar la burbuja detrás del icono
        const leftPos = itemRect.left - navRect.left + (itemRect.width - bubbleWidth) / 2;
...
function checkDeepLinks() {
    ...
    if (urlParams.has('movie')) {
        ...
        if (parts.includes('vip')) {
            ...
            // Verifica que provenga estrictamente de linkvertise
        ...
        if (parts.includes('vip')) {
            ...
            // Chequeo de varios dominios de linkvertise comunes
            if (!ref.includes('linkvertise.com') && !ref.includes('link-center.net') && !ref.includes('link-target.net') && !ref.includes('link-hub.net')) {
                showToast("Error V4: Acceso no autorizado");
                window.history.replaceState({}, '', window.location.href.replace('-vip', ''));
            } else {
...
window.executeUnlock = function() {
    if (currentModalItem) {
        ...
        if (shortLink) {
            ...
            showToast("Enlace de desbloqueo no disponible.");
        }
    }
};
...
function switchView(viewName, pushToHistory = true) {
    ...
    // Mover la animación del cristal a la pestaña correcta
    updateNavIndicator();
