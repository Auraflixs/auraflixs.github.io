...
return `<div class="item img-loader" data-id="${safeId}" onclick="openModal('${safeId}', '${type}')">   <img src="${item.image}"    class="smooth-image"    onload="this.parentElement.classList.remove('img-loader'); this.classList.add('loaded')"    loading="${loadMode}"    decoding="async"    alt="${item.title}">   <div class="item-title">${item.title}</div>   </div>`;
}
...
// Si estamos en PELÍCULAS o SERIES, usamos el grid normal que responde a las opciones de 3 o 4 columnas
const isHome = containerId === 'customCategoriesContainer';
const rowClass = isHome ? 'row horizontal-scroll' : 'row grid-layout';
