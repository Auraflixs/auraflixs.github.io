...
function setupEventListeners() {
    ...
    // --- NUEVO: EVENTOS PARA MODAL DE SOLICITUD ---
    ...
    // --- NUEVO: EVENTOS PARA MODAL DE REPORTE ---
    // Este evento prepara el formulario de reporte automáticamente
    document.getElementById('modalReportBtn').onclick = () => {
        if (!currentModalItem) return;
        
        let reportTitle = currentModalItem.title;
        
        // Si es una serie, intentamos detectar qué temporada y capítulo está activo
        if (currentModalItem.seasons) {
            const season = currentModalItem.seasons[currentSeasonIndex];
            if (season) {
                reportTitle += ` (Temporada ${season.season}`;
                
                // Buscamos el botón de episodio que esté "active"
        ...
        // Rellenamos el título visible y el input oculto
