...
// Estas variables se inicializan DENTRO de DOMContentLoaded para asegurar carga de datos
...
document.addEventListener('DOMContentLoaded', () => {
    // 1. CARGA SEGURA DE DATOS: Esperamos a que el navegador tenga todo listo
    ...
    if(moviesListInternal.length === 0 && seriesListInternal.length === 0) {
        console.warn("Auraflix: No se encontraron películas ni series. Verifica los archivos JS externos.");
    }

    checkLoginStatus();
    // Limpiamos el historial para evitar modales abiertos al recargar
...
function initApp() {
    ...
    // Combinamos todo para tener contenido si la secuencia manual falla
    const allContent = [...moviesListInternal, ...seriesListInternal];
    
    if (window.allContentSequence && window.allContentSequence.length > 0) {
        // Mapeamos los IDs de la secuencia a objetos reales
        const sequenceItems = window.allContentSequence.map(id => {
            return allContent.find(item => String(item.id) === String(id));
        }).filter(item => item !== undefined); // Filtramos nulos
        
        renderList('newlyAddedRow', sequenceItems.reverse().slice(0, 20));
    } else {
        // Fallback: mostrar los últimos items del array global
...
function setupHero() {
    ...
    // 1. Prioridad: Lista de IDs destacados manual
    ...
    // 2. Fallback: Buscar propiedad .featured en los objetos
    if (featuredList.length === 0) {
        const allFeatured = allContent.filter(i => i.featured);
        featuredList = allFeatured.length > 0 ? allFeatured.slice(0, 5) : [];
    }
    
    // 3. Fallback final: Tomar los primeros 5 elementos cualesquiera
...
function renderHomeView() {
    // Clonamos para no alterar el orden original
...
function openModal(id, typeHint) {
    ...
    // 3. BÚSQUEDA SEGURA DEL ITEM
    const idStr = String(id);
    const allContent = [...moviesListInternal, ...seriesListInternal];
    const item = allContent.find(i => String(i.id) === idStr);
    
    if (!item) {
        console.error("Error: No se encontró contenido con ID", id);
        return;
    }

    currentModalItem = item;
    
    // Detectamos si es serie (tiene temporadas) o película
    const isSeries = (item.seasons && item.seasons.length > 0) || typeHint === 'series';
    currentModalType = isSeries ? 'series' : 'movies';
    ...
    // LÓGICA DE GÉNEROS: Si existe, mostrar. Si no, ocultar separador.
    ...
    // 6. CONFIGURACIÓN PLAYER Y EPISODIOS
    if (isSeries) {
...
function renderRealRecommendations(currentId) {
    ...
    // Filtramos para no mostrar la misma que se está viendo
    ...
    container.innerHTML = selection.map(item => {
        // Forzamos el tipo correcto para que el click funcione bien
        const type = item.seasons ? 'series' : 'movies';
        // Escapamos comillas simples en IDs extraños para evitar errores de sintaxis en el HTML
...
function setupEventListeners() {
    ...
    // Exponer renderAvatarSelection al scope global si es necesario o usar dentro del closure
    window.renderAvatarSelection = renderAvatarSelection;
    ...
    document.getElementById('confirmAvatarChange').addEventListener('click', () => {
        if(!selectedAvatarTemp) { alert("Selecciona una imagen primero."); return; }
    ...
    document.getElementById('logoutBtn').addEventListener('click', () => {
        if(confirm("¿Cerrar sesión y borrar todos los datos?")) { localStorage.clear(); location.reload(); }
    });
}

function renderAvatarSelection(containerId, context) {
    // Definida dentro de setupEventListeners para acceso scope, pero también aquí para acceso desde checkLoginStatus
    const grid = document.getElementById(containerId);
    if(!grid) return;
    grid.innerHTML = '';
    if (typeof profileImages !== 'undefined') {
