...
// Verificamos si la página ya cargó antes de intentar iniciar
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', iniciarTodo);
} else {
    // Si ya está lista (loading terminó), iniciamos directamente
    iniciarTodo();
}
...
function iniciarTodo() {
    ...
    try {
        ...
        console.log("History API no disponible localmente");
    }
...
function initApp() {
    ...
    // ORDEN: Usamos globalContent para respetar el orden exacto de carga
    let orderedContent = [];
...
function setupHero() {
    ...
    // SI EXISTE HERO_IDS, USAMOS SOLO ESO.
    if (window.HERO_IDS && Array.isArray(window.HERO_IDS) && window.HERO_IDS.length > 0) {
        ...
        // Solo si NO hay HERO_IDS definidos, usamos el fallback
