...
window.addEventListener('load', () => {
    // Si estamos usando el archivo de enlaces, verificamos si existe
...
function iniciarTodo() {
    // 1. Inyectamos los estilos para la carga suave de imágenes
    injectImageStyles();
...
// ==========================================
// NUEVO: ESTILOS DE CARGA SUAVE (FADE IN)
// ==========================================
function injectImageStyles() {
    ...
    const css = `
        ...
        /* Cuando cargan, se vuelven visibles suavemente */
        .smooth-image.loaded {
            opacity: 1;
        }
        
        /* Asegurar que el contenedor del avatar tenga overflow hidden */
        .avatar-option img {
            opacity: 0;
...
function renderAvatarSelection(containerId, context) {
    ...
    if (typeof profileImages !== 'undefined' && Array.isArray(profileImages)) {
        profileImages.forEach((url) => {
            ...
            // APLICAMOS CARGA SUAVE AL AVATAR
...
function handleImageUpload(e) {
    ...
    reader.onload = function(event) {
        ...
        if (activeGridId && activeBtnId) {
            ...
            grid.querySelectorAll('.avatar-option').forEach(el => el.classList.remove('selected'));
            if(btn) {
                // APLICAMOS CARGA SUAVE AL AVATAR SUBIDO
...
function createItemHTML(item) {
    ...
    // APLICAMOS CARGA SUAVE A LAS PORTADAS
    // La clase smooth-image la hace invisible al inicio
    // onload agrega la clase 'loaded' que la hace visible suavemente
    return `
