...
function injectImageStyles() {
  ...
  /* Asegurar que la imagen respete el aspect-ratio normal */
  ...
  /* Reducir espacio entre la línea separadora y el título de categoría (ej. "Recomendados") */
  #movieCategoriesContainer .section-title,
  #seriesCategoriesContainer .section-title,
  .section-title {
    margin: 6px 4% 4px; /* top 6px (antes ~15px), bottom 4px (antes ~10px) */
  }

  /* Evitar padding extra en las filas horizontales que empuje hacia abajo */
  .row.horizontal-scroll.category-horizontal { padding-top: 0; margin-top: 0; }

  /* Ajustes para dispositivos grandes: mantener proporcionalidad */
...
function initApp() {
  ...
  // Ordenar por globalid de mayor a menor (descendente)
  newlyAdded.sort((a, b) => {
  ...
  // Llamada a la función de categorías  
...
function checkDeepLinks() {
  ...
  if (urlParams.has('movie')) {
    ...
    if (parts.includes('vip')) {
      ...
      const cameFromOuo = (document.referrer && document.referrer.includes('ouo.io'));
      if (ticketTime && (now - parseInt(ticketTime)) < 1800000 && cameFromOuo) {
...
function createItemHTML(item, index = 100, sizeMode = null) {
  ...
  // compute inline width style only when sizeMode === 'category-horizontal'
  let inlineStyle = '';
  if (sizeMode === 'category-horizontal') {
      // If body has grid-small class, use 4-column calc, otherwise 3-column calc
...
function closeModalInternal() {
  ...
  if (cameFromOuo || isVipUrl) {  
      window.history.replaceState({ view: 'home', modal: false, search: false, history: false }, '', baseUrl);  
      switchView('home', false);  
  } else {  
      window.history.replaceState({ view: currentView, modal: false, search: wasSearchOpen, history: wasHistoryOpen }, '', baseUrl);  
  }
}
...
// --- SISTEMA DINÁMICO DE CATEGORÍAS (Para Home, Películas y Series) ---
function renderCustomCategories() {
  const allContent = [...window.moviesList, ...window.seriesList];

  // 1. Categorías en la sección INICIO  
  ...
  // 2. Categorías en la sección PELÍCULAS  
  ...
  // 3. Categorías en la sección SERIES  
...
function renderCategoryGroup(categoriesData, containerId, contentList, parentId = null, insertBeforeId = null) {
  ...
  // Crear el contenedor dinámicamente si no existe en el HTML  
  ...
  // Si estamos en la sección INICIO (home), mantenemos el scroll horizontal
  // Si estamos en PELÍCULAS o SERIES, queremos mostrar las categorías en horizontal
  // pero usando el tamaño de portada correspondiente (3 o 4 columnas).
  const isHome = containerId === 'customCategoriesContainer';
  // Para Movies/Series usamos horizontal-scroll pero con la clase category-horizontal
  ...
  categoriesData.forEach(category => {
      ...
      if (categoryItems.length > 0) {
          ...
          // cuando NO es home, pedimos a createItemHTML que genere items con tamaño 'category-horizontal'
          if (isHome) {
  ...
  // Agregamos una línea de separación elegante solo en las vistas de Películas y Series
  if (htmlOutput !== '' && !isHome) {
      // Reducimos márgenes arriba/abajo para que no quede tanto espacio
      // <-- Aquí hicimos el cambio solicitado: bottom margin reducido a la mitad (8px -> 4px)
      htmlOutput += `<div style="width: 92%; height: 1px; background-color: #333; margin: 15px auto 4px auto;"></div>`;
  }
...
/* ---------------------------
   CONFIG: etiquetas de Novedades
   ---------------------------
   El bloque siguiente añade badges según NEW_CONTENT_CONFIG,
   pero evita añadir badges dentro de #movieCategoriesContainer y #seriesCategoriesContainer
   (así en las vistas de Películas/Series no se mostrarán etiquetas).
*/
...
(function() {
    function applyNewBadges() {
        ...
        NEW_CONTENT_CONFIG.forEach(config => {
            ...
            items.forEach(item => {
                // NUEVO: ignorar items que estén dentro de las categorías de Películas/Series
                // (si están dentro de esos contenedores no añadimos badge)
                const insideMoviesCats = item.closest && item.closest('#movieCategoriesContainer');
                const insideSeriesCats = item.closest && item.closest('#seriesCategoriesContainer');
                if (insideMoviesCats || insideSeriesCats) return; // skip: no badge en vistas de categorías
