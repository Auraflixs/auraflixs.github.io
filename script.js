...
// Variables para el sistema de diagnóstico
...
// --- ARRANQUE SEGURO (Estructura Original Restaurada) ---
...
function initApp() {
    // Mezcla aleatoria solo una vez al iniciar
...
function loadUserDataInUI() {
    ...
    // Solo mostramos las herramientas si el usuario es uno de los elegidos
    const adminPanel = document.getElementById('adminTools');
    if (adminPanel) {
        const admins = ['Naho-ad', 'L-ad'];
...
function openModal(id, typeHint) {
    ...
    if (isSeries) {
        ...
        if (item.seasons && item.seasons.length > 0) {
            ...
            // Aquí la magia: Forzamos índice 0 (capítulo 1)
        ...
        // Verificamos VIP para reproducir película
        // Dependencia de vip.js: function isUserVip()
        ...
        actionBtn.onclick = () => {
            if (isPlayingTrailer) {
                // Volver a película -> CHEQUEAR VIP
...
function openSeasonSelectorModal() {
    ...
    currentModalItem.seasons.forEach((seasonObj, index) => {
        ...
        itemBtn.onclick = () => {
            ...
            // Al cambiar temporada, forzamos inicio en Capítulo 1 (index 0)
...
function renderEpisodes(season, serieItem, autoPlayIndex = -1) {
    ...
    // Si hay un capítulo preseleccionado (al abrir modal o cambiar temp)
    if (autoPlayIndex >= 0 && season.episodes[autoPlayIndex]) {
        if(isVip) {
            ...
            // Si no es VIP, mostramos el bloqueo en el reproductor inmediatamente
            showVipBlocker();
        }
    }
    
    // Listeners para botones de episodios
    ...
    buttons.forEach((btn, index) => {
        btn.onclick = () => {
            ...
            // CHECK VIP AL CLICAR CUALQUIER EPISODIO
            const checkVip = typeof isUserVip === 'function' ? isUserVip() : false;
...
async function startDiagnosis(mode) {
    let confirmMsg = "";
    if (mode === 'content') confirmMsg = "¿Verificar contenido principal (Películas/Capítulos)?";
                ...
                s.seasons.forEach(season => {
                    if (season.episodes) {
                        season.episodes.forEach(ep => {
                            tasks.push({ 
                                type: 'episode', 
                                // FORMATO SOLICITADO: Serie (T1:E1)
...
async function checkLink(item, logContainer) {
    ...
    try {
        ...
        // Si no explota, asumimos OK y no mostramos nada (solo errores)

    } catch (error) {
        if (error.name === 'AbortError') {
            logToConsole(logContainer, item.title, "Timeout (Lento)", "warning");
        } else {
            logToConsole(logContainer, item.title, "Error / Caído", "error");
        }
    }
}
...
async function checkVimeoOfficial(title, videoUrl, container) {
    try {
        ...
        // Errores de red puntuales se ignoran
    }
}
