...
function setupHero() {
    ...
    if (window.HERO_IDS && Array.isArray(window.HERO_IDS) && window.HERO_IDS.length > 0) {
        const first = window.HERO_IDS[0];
        if (typeof first === 'object' && first !== null) {
            // Asumimos que son objetos directos
            featuredList = window.HERO_IDS.filter(item => item && typeof item === 'object');
        } else {
            // Asumimos que son IDs (strings o números)
            featuredList = window.HERO_IDS
                .map(id => allContent.find(item => String(item.id) === String(id)))
...
function switchView(viewName, pushToHistory = true) {
    ...
    if (pushToHistory) window.history.pushState({ view: viewName }, '');
}
...
function closeVideoModal() {
    ...
    isPlayingTrailer = false;
    document.getElementById('seriesEpisodeSelector').classList.add('hidden');
...
function setupEventListeners() {
    ...
    window.addEventListener('popstate', (event) => {
        closeVideoModal();
        closeSearchOverlay();
        if (event.state && event.state.view && event.state.view !== currentView) {
