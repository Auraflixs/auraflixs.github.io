...
document.getElementById('btnBroadcast').onclick = () => {
    window.open('https://t.me/Auraflixpeli', '_blank');
};
...
document.getElementById('btnShare').onclick = () => {
    ...
    if (navigator.share) {
        navigator.share({
            title: 'Auraflix',
            text: 'Mira películas y series increíbles.',
            url: link,
        }).catch(console.error);
    } else {
...
document.getElementById('logoutBtn').addEventListener('click', () => {
    if(confirm("¿Cerrar sesión y borrar todos los datos?")) {
        localStorage.clear();
        location.reload();
    }
});
...
window.addEventListener('popstate', (event) => {
    ...
    if (!event.state || !event.state.modal) {
        ...
        if (supportModal.style.display === 'flex') supportModal.style.display = 'none';
    }
...
function openModal(id, type) {
    ...
    const metaHighlight = document.getElementById('modalMetaHighlight');
    ...
    const typeLabel = type === 'movies' ? 'Película' : 'Serie';
    
    // --- EDICIÓN REALIZADA AQUÍ ---
    // Año y Etiqueta en Morado (según CSS .meta-highlight)
    metaHighlight.innerText = `${item.year} ${typeLabel}`;

    // Géneros y punto en Gris (según CSS .meta-genres)
    if (item.genres && item.genres.length > 0) {
        let displayGenres = item.genres.slice(0, 4).join('/');
        if (item.genres.length > 4) {
            displayGenres += '...';
        }
        metaGenres.innerText = " • " + displayGenres;
    } else {
        metaGenres.innerText = "";
    }
    // -----------------------------
