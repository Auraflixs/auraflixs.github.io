...
// AQUÍ AGREGUÉ EL DATA-ID PARA PODER CREAR LA ANIMACIÓN HERO
...
function openModal(id, typeHint, fromDeepLink = false) {
    ...
    // ==========================================================
    // INICIO DE LA ANIMACIÓN HERO (SHARED ELEMENT TRANSITION)
    // ==========================================================
    ...
    if (clickedCard && !fromDeepLink) {
        ...
        modal.style.display = 'flex';
        playerWrapper.style.opacity = '0'; // Ocultamos el reproductor real temporalmente
        
        // Forzamos al navegador a registrar los estilos antes de animar
        requestAnimationFrame(() => {
            const targetRect = playerWrapper.getBoundingClientRect();

            // Matemática FLIP para que salga exactamente desde donde está la tarjeta
            ...
            // Posicionamos el clon en el destino final pero lo encogemos y movemos al origen
            ...
            // Activamos la expansión de la imagen hacia la pantalla completa
            requestAnimationFrame(() => {
                clone.style.transform = 'translate(0, 0) scale(1, 1)';
                clone.style.borderRadius = '0px';
                modal.classList.add('active'); // Fondo a negro

                // Hacemos que la info de la película suba flotando
                setTimeout(() => {
                    modal.classList.add('open');
                }, 50);

                // Limpieza al terminar la transición (400ms)
                setTimeout(() => {
        ...
        // Apertura estándar sin animación si recargan la página directa
        modal.style.display = 'flex';
        playerWrapper.style.opacity = '1';
