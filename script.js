...
function blockPlayerDynamically() {
    if (!currentPlayerIframe) return;
    currentPlayerIframe.style.pointerEvents = 'none';
    if (currentPlayerIframe.contentWindow && currentPlayerOrigin) {
        currentPlayerIframe.contentWindow.postMessage({ command: 'pause' }, currentPlayerOrigin);
    }
    const container = currentPlayerIframe.parentElement;
    if (!container) return;
    let blockerDiv = container.querySelector('.vip-player-overlay');
    if (!blockerDiv) {
        ...
        blockerDiv.innerHTML = '<i class="fas fa-lock vip-lock-icon"></i><p class="vip-overlay-text" style="margin-bottom: 25px;">El minuto gratuito ha terminado</p><div style="display: flex; flex-direction: column; width: 85%; max-width: 250px;">' + buttonsHtml + '</div>'; 
...
function installPlayerMessageHandler() { 
    ...
    window.addEventListener('message', (e) => { 
        ...
        if (currentTime !== undefined) {
            window.__auraCurrentPlayerTime = currentTime;
            if (window.__auraIsContentRestricted && !window.__auraIsCurrentlyBlocked && !isPlayingTrailer) {
                if (window.__lastTime !== undefined && currentTime > window.__lastTime) {
                    const diff = currentTime - window.__lastTime;
                    if (diff > 0 && diff < 3) {
                        window.__accumulatedPlayTime += diff;
                    }
                }
                window.__lastTime = currentTime;
                if (window.__accumulatedPlayTime >= 60) {
                    window.__auraIsCurrentlyBlocked = true;
                    blockPlayerDynamically();
                }
            }
        }
        ...
        if (window.__auraHasSeeked && typeof currentTime === 'number' && currentPlayerProgressKey && !window.__auraIsCurrentlyBlocked) { 
...
function openModal(id, typeHint, fromDeepLink = false) {
    ...
    if (isSeries) {
        ...
        actionBtn.onclick = () => {  
            ...
            setPlayerVideo(item.trailer, "Tráiler", true);  
...
function setPlayerVideo(url, overlayText = null, hasAccess = false) { 
    ...
    window.__seekingNow = false;
    window.__accumulatedPlayTime = 0;
    window.__lastTime = undefined;
    window.__auraIsCurrentlyBlocked = false;
    
    const needsRestriction = !hasAccess && !isPlayingTrailer;
    window.__auraIsContentRestricted = needsRestriction;
...
function closeModalInternal() {
    ...
    window.__lastTime = undefined;
    window.__auraIsCurrentlyBlocked = false;
    window.__auraIsContentRestricted = false;
