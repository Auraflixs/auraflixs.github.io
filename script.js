...
window.enteredViaDeepLink = false; // FLAG para saber si entramos por un link directo

// --- EFECTO MAQUINA DE ESCRIBIR PARA EL LOGO ---
const textToType = "uraflix+";
let typingIndex = 0;
...
function typeWriterEffect() {
    ...
    if (!isDeleting && typingIndex <= textToType.length) {
        typingEl.innerText = textToType.substring(0, typingIndex);
        typingIndex++;
        ...
        if(logo) logo.classList.toggle('typing', !isDeleting);
        setTimeout(typeWriterEffect, isDeleting ? 3000 : 800); 
    }
}

// --- BLOQUE: MEMORIA DE TIEMPO DEL REPRODUCTOR ---
...
window.__auraCurrentPlayerTime = 0; // Para adelantar/retrasar
...
function installPlayerMessageHandler() { 
    ...
    window.addEventListener('message', (e) => { 
        ...
        const isReady = data.playerStatus === 'Ready' || data.playerStatus === 'Listo' || data.event === 'ready' || data.event === 'loaded' || data.event === 'loadedmetadata'; 
        if (isReady && currentPlayerProgressKey) { 
            const savedTime = localStorage.getItem(currentPlayerProgressKey); 
            if (savedTime && currentPlayerIframe.contentWindow) { 
                currentPlayerIframe.contentWindow.postMessage( 
                    { command: 'seek', value: parseFloat(savedTime) }, 
                    '*' 
                ); 
            } 
            ...
            window.__auraPlayerTimeInterval = setInterval(() => { 
        ...
        const currentTime = typeof data.currentTime === 'number' ? data.currentTime : (typeof data.time === 'number' ? data.time : undefined); 
        const duration = typeof data.duration === 'number' ? data.duration : undefined; 
        if (typeof currentTime === 'number') {
            window.__auraCurrentPlayerTime = currentTime; // Guardar tiempo en vivo para botones de 10s
        }
...
function setUnlockedForTicket(ticketKey, hours = 24) {
...
function iniciarTodo() {
    injectImageStyles();
    optimizeConnections();
    typeWriterEffect(); // Iniciar animación de LOGO
...
function initApp() {
    ...
    newlyAdded.sort((a, b) => {
        const idA = parseInt(a.globalid) || 0;
        const idB = parseInt(b.globalid) || 0;
        return idB - idA;
    });
...
function checkDeepLinks() {
    ...
    if (urlParams.has('movie') || urlParams.has('serie')) {
        window.enteredViaDeepLink = true; // Flag que indica acceso directo
    }
...
function renderHero() {
    ...
    container.innerHTML = featuredList.map((item, i) => `<div class="carousel-slide ${i === 0 ? 'active' : ''} img-loader">   <img src="${item.image}"    alt="Hero Image"    class="smooth-image"    onload="this.parentElement.classList.remove('img-loader'); this.classList.add('loaded')"   ${i === 0 ? 'loading="eager" fetchpriority="high"' : 'loading="lazy"'}    decoding="async">   </div>`).join('');
...
function setupLatelyNew() {
    ...
    const allContent = [...sourceMovies, ...sourceSeries];
    let displayList = [];
    targetIds.forEach(entry => {
        let id = null;
        let overrideImg = null;
        if (typeof entry === 'object' && entry !== null) {
            id = entry.id;
            overrideImg = entry.img || null;
        } else {
            id = entry;
        }
        if (!id) return;
...
function createItemHTML(item, index = 100, sizeMode = null) {
...
function openModal(id, typeHint, fromDeepLink = false) {
    ...
    if (!document.body.classList.contains('modal-open') && !fromDeepLink) {
        window.history.pushState({ view: currentView, modal: true, search: wasSearchOpen, history: wasHistoryOpen }, '');
    }
    ...
    if (isSeries) {
        ...
        const hasAccess = (window.VIP_SYSTEM_STATUS === 'off' && window.UNLOCK_LINK_STATUS === 'off')  
            ? true  
            : (isVipGlobal || isUnlockedViaLink || isUnlockedViaMemory);  
        ...
        setPlayerVideo(item.video, item.title, !hasAccess);  
...
function renderEpisodes(season, serieItem, autoPlayIndex = -1) {
    ...
    container.innerHTML = season.episodes.map((ep, idx) => `<button class="episode-button ${idx === autoPlayIndex ? 'active' : ''}" data-idx="${idx}">${ep.episode}</button>`).join('');
    ...
    if (autoPlayIndex >= 0 && season.episodes[autoPlayIndex]) {
        ...
        setPlayerVideo(ep.video, `${serieItem.title} - T${season.season} E${ep.episode}`, !hasAccess);  
...
function buildUnlockedDestUrlForCurrentItem() {
    ...
    if (!currentModalItem.seasons) {
        const year = currentModalItem.year || currentYear;
        const dest = `${base}?movie=${currentModalItem.id}-${year}-vip`;
        return dest;
    } else {
        ...
        if (!epNumber && seasonObj.episodes && seasonObj.episodes.length > 0) {
            epNumber = seasonObj.episodes[0].episode;
        }
        if (!epNumber) epNumber = 1;
        const dest = `${base}?serie=${currentModalItem.id}-t${seasonObj.season}-e${epNumber}-vip`;
        return dest;
    }
}
...
window.executeUnlock = function() {
    ...
    if (dest) {
        const redirectTo = OUO_PREFIX + encodeURIComponent(dest);
...
// --- SETPLAYERVIDEO ACTUALIZADO CON PANTALLA COMPLETA PERSONALIZADA ---
function setPlayerVideo(url, overlayText = null, isBlocked = false) { 
    ...
    if (playerDiv.firstChild && playerDiv.firstChild._fsListener) {
        document.removeEventListener('fullscreenchange', playerDiv.firstChild._fsListener);
        document.removeEventListener('webkitfullscreenchange', playerDiv.firstChild._fsListener);
    }
    ...
    container.style.backgroundColor = '#000'; // Fondo negro para fullscreen
    ...
    try { 
        currentPlayerOrigin = new URL(finalUrl).origin; 
    } catch (err) { 
    ...
    if (window.VIP_SYSTEM_STATUS === 'off' && window.UNLOCK_LINK_STATUS === 'off') { 
        isBlocked = false; 
    } 
    if (isBlocked) { 
    ...
    // --- INTERFAZ OVERLAY DEL REPRODUCTOR (CONTROLES CUSTOM) ---
    const uiContainer = document.createElement('div');
    uiContainer.style.position = 'absolute';
    uiContainer.style.inset = '0';
    uiContainer.style.pointerEvents = 'none'; // Deja pasar los clicks al iframe por defecto
    uiContainer.style.zIndex = '100';

    const safeTitle = overlayText || (currentModalItem ? currentModalItem.title : 'Auraflix');

    uiContainer.innerHTML = `
        <div id="auraPlayerOverlay" style="display:none; width:100%; height:100%; position:relative; pointer-events:none; transition: opacity 0.3s ease;">
            <div style="position:absolute; top:0; left:0; right:0; padding:15px; background:linear-gradient(to bottom, rgba(0,0,0,0.85), transparent); display:flex; align-items:center; gap:15px; pointer-events:auto; z-index:102;">
                <button id="auraFsBack" style="background:none; border:none; color:white; font-size:1.5rem; cursor:pointer;"><i class="fas fa-arrow-left"></i></button>
                <span style="color:white; font-size:1.15rem; font-weight:bold; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; text-shadow: 1px 1px 3px rgba(0,0,0,0.8);">${safeTitle}</span>
            </div>
            
            <div id="auraSpeedText" style="display:none; position:absolute; top:15%; left:50%; transform:translateX(-50%); background:rgba(138,43,226,0.9); color:white; padding:5px 15px; border-radius:20px; font-weight:bold; font-size:1.2rem; z-index:102; box-shadow: 0 4px 10px rgba(0,0,0,0.5);">x2</div>
            
            <div style="position:absolute; top:50%; left:50%; transform:translate(-50%, -50%); display:flex; gap:80px; pointer-events:auto; z-index:101;">
                <button id="auraRewind" style="background:rgba(0,0,0,0.55); border:none; color:white; border-radius:50%; width:65px; height:65px; font-size:1.4rem; cursor:pointer; display:flex; flex-direction:column; align-items:center; justify-content:center; backdrop-filter: blur(4px); transition: 0.2s;"><i class="fas fa-undo"></i><span style="font-size:0.75rem; font-weight:bold; margin-top:2px;">10</span></button>
                <button id="auraForward" style="background:rgba(0,0,0,0.55); border:none; color:white; border-radius:50%; width:65px; height:65px; font-size:1.4rem; cursor:pointer; display:flex; flex-direction:column; align-items:center; justify-content:center; backdrop-filter: blur(4px); transition: 0.2s;"><i class="fas fa-redo"></i><span style="font-size:0.75rem; font-weight:bold; margin-top:2px;">10</span></button>
            </div>

            <div id="auraZoneLeft" style="position:absolute; top:60px; left:0; width:120px; height:calc(100% - 120px); pointer-events:auto; z-index:101;"></div>
            <div id="auraZoneRight" style="position:absolute; top:60px; right:0; width:120px; height:calc(100% - 120px); pointer-events:auto; z-index:101;"></div>
        </div>
        
        <button id="auraEnterFs" style="position:absolute; bottom:15px; right:15px; background:rgba(0,0,0,0.6); border:none; color:white; border-radius:8px; padding:10px 12px; font-size:1.2rem; cursor:pointer; pointer-events:auto; z-index:90; backdrop-filter: blur(4px); transition: 0.2s;"><i class="fas fa-expand"></i></button>
    `;
    
    container.appendChild(uiContainer);

    // -- LÓGICA DE LA INTERFAZ --
    const auraPlayerOverlay = uiContainer.querySelector('#auraPlayerOverlay');
    const auraFsBack = uiContainer.querySelector('#auraFsBack');
    const auraEnterFs = uiContainer.querySelector('#auraEnterFs');
    const auraRewind = uiContainer.querySelector('#auraRewind');
    const auraForward = uiContainer.querySelector('#auraForward');
    const auraSpeedText = uiContainer.querySelector('#auraSpeedText');
    const auraZoneLeft = uiContainer.querySelector('#auraZoneLeft');
    const auraZoneRight = uiContainer.querySelector('#auraZoneRight');

    let uiTimeout;
    const resetUiHide = () => {
         auraPlayerOverlay.style.opacity = '1';
         clearTimeout(uiTimeout);
         uiTimeout = setTimeout(() => { auraPlayerOverlay.style.opacity = '0'; }, 3000);
    };

    // Reactivar UI al moverse / tocar
    auraPlayerOverlay.addEventListener('mousemove', resetUiHide);
    auraPlayerOverlay.addEventListener('touchstart', resetUiHide, {passive: true});

    // Enviar comandos al iframe
    const sendPlayerCommand = (command, value) => {
        if (currentPlayerIframe && currentPlayerIframe.contentWindow) {
            currentPlayerIframe.contentWindow.postMessage({ command, value }, '*');
        }
    };
    ...
    const handleFsChange = () => {
         if (document.fullscreenElement === container || document.webkitFullscreenElement === container) {
             auraPlayerOverlay.style.display = 'block';
             auraEnterFs.style.display = 'none';
             resetUiHide();
         } else {
             auraPlayerOverlay.style.display = 'none';
             auraEnterFs.style.display = 'block';
             clearTimeout(uiTimeout);
         }
    };
    document.addEventListener('fullscreenchange', handleFsChange);
    document.addEventListener('webkitfullscreenchange', handleFsChange);
    container._fsListener = handleFsChange; // Se asocia para limpiarlo luego

    auraRewind.onclick = () => {
         const newTime = Math.max(0, (window.__auraCurrentPlayerTime || 0) - 10);
         sendPlayerCommand('seek', newTime);
         resetUiHide();
    };
    auraForward.onclick = () => {
         const newTime = (window.__auraCurrentPlayerTime || 0) + 10;
         sendPlayerCommand('seek', newTime);
         resetUiHide();
    };
    ...
    const startSpeed = (e) => {
         ...
         auraSpeedText.style.display = 'block';
         sendPlayerCommand('setPlaybackRate', 2);
         resetUiHide();
    };
    const stopSpeed = (e) => {
         if(e.cancelable) e.preventDefault();
         auraSpeedText.style.display = 'none';
         sendPlayerCommand('setPlaybackRate', 1);
    };
    
    [auraZoneLeft, auraZoneRight].forEach(zone => {
         zone.addEventListener('mousedown', startSpeed);
         zone.addEventListener('mouseup', stopSpeed);
         zone.addEventListener('mouseleave', stopSpeed);
         zone.addEventListener('touchstart', startSpeed, {passive: false});
         zone.addEventListener('touchend', stopSpeed);
         zone.addEventListener('touchcancel', stopSpeed);
    });

    // Si es VIP / Bloqueado oculta interfaz extra
    if (isBlocked) { 
        ...
        uiContainer.style.display = 'none'; // Oculta UI interactiva
    } 
    playerDiv.appendChild(container); 
}
// --- FIN BLOQUE REPRODUCTOR ---
...
function closeModalInternal() {
    ...
    if (player) {
        // Limpiamos los eventos del Fullscreen del reproductor
        if (player.firstChild && player.firstChild._fsListener) {
            document.removeEventListener('fullscreenchange', player.firstChild._fsListener);
            document.removeEventListener('webkitfullscreenchange', player.firstChild._fsListener);
        }
        player.innerHTML = ''; 
    }
    if (window.__auraPlayerTimeInterval) { 
...
function renderPopularSearches() {
    ...
    container.innerHTML = window.busquedasPopulares.map(term => `<span class="pop-tag">${term}</span>`).join('');
...
function setupEventListeners() {
    ...
    if(navProfile) navProfile.onclick = (e) => { e.preventDefault(); switchView('profile'); };
    ...
    if(btnFilterMovies) btnFilterMovies.onclick = () => openFilterModal('movies');
    if(btnFilterSeries) btnFilterSeries.onclick = () => openFilterModal('series');
    ...
    if(closeFilterModal) closeFilterModal.onclick = () => document.getElementById('filterModal').style.display = 'none';
    ...
    if(closeSearch) closeSearch.onclick = () => { wasSearchOpen = false; window.history.back(); };

    const closeHistory = document.getElementById('closeHistory');
    if(closeHistory) closeHistory.onclick = () => { wasHistoryOpen = false; window.history.back(); };

    // --- MANEJO OPTIMIZADO PARA NO SACAR DEL SITIO (DEEP LINKS FIX) ---
    const closeModal = document.getElementById('closeModal');
    if(closeModal) closeModal.addEventListener('click', () => {
        closeModalInternal();
        const cameFromOuo = (document.referrer && (document.referrer.includes('ouo.io') || document.referrer.includes('ouo.press')));
        const isVipUrl = window.location.href.includes('-vip');
        
        // Si no hay historial base o vino de un enlace directo, limpiamos y nos quedamos en Inicio
        if (window.enteredViaDeepLink || cameFromOuo || isVipUrl || window.history.length <= 1) {
            window.history.replaceState({ view: 'home', modal: false, search: false, history: false }, '', window.location.pathname);
    ...
    if(closeSeasonSelector) closeSeasonSelector.onclick = () => document.getElementById('seasonSelectorModal').style.display = 'none';
    ...
    if(profilePageImg) {
        profilePageImg.addEventListener('click', () => {
            document.getElementById('changeAvatarModal').style.display = 'flex';
            renderAvatarSelection('changeAvatarGrid', 'modal');
        });
    }
    const closeAvatarModal = document.getElementById('closeAvatarModal');
    if(closeAvatarModal) closeAvatarModal.addEventListener('click', () => document.getElementById('changeAvatarModal').style.display = 'none');
    const confirmAvatarChange = document.getElementById('confirmAvatarChange');
    if(confirmAvatarChange) confirmAvatarChange.addEventListener('click', () => {
    ...
    if(btnSupport) btnSupport.onclick = () => document.getElementById('supportModal').style.display = 'flex';
    const closeSupportBtn = document.getElementById('closeSupportBtn');
    if(closeSupportBtn) closeSupportBtn.onclick = () => document.getElementById('supportModal').style.display = 'none';
    ...
    if(btnBroadcast) {
        btnBroadcast.onclick = () => {
            if(typeof AURA_LINKS !== 'undefined' && AURA_LINKS.telegram) {
                window.location.href = AURA_LINKS.telegram;
            }
        };
    }

    const btnTerms = document.getElementById('btnTerms');
    if(btnTerms) btnTerms.onclick = () => document.getElementById('termsModal').style.display = 'flex';
    const closeTermsBtn = document.getElementById('closeTermsBtn');
    if(closeTermsBtn) closeTermsBtn.onclick = () => document.getElementById('termsModal').style.display = 'none';

    const btnSettings = document.getElementById('btnSettings');
    if(btnSettings) btnSettings.onclick = () => document.getElementById('settingsModal').style.display = 'flex';
    const closeSettingsBtn = document.getElementById('closeSettingsBtn');
    if(closeSettingsBtn) closeSettingsBtn.onclick = () => document.getElementById('settingsModal').style.display = 'none';

    const btnShare = document.getElementById('btnShare');
    if(btnShare) {
        btnShare.onclick = () => document.getElementById('shareModal').style.display = 'flex';
    }
    const closeShareModal = document.getElementById('closeShareModal');
    if(closeShareModal) {
        closeShareModal.onclick = () => document.getElementById('shareModal').style.display = 'none';
    }
    ...
    if(btnShareApp) {
        btnShareApp.onclick = () => {
            ...
            shareGeneric("Descarga Auraflix App", url);
    ...
    if(btnRequestContent) btnRequestContent.onclick = () => { document.getElementById('requestModal').style.display = 'flex'; };
    const closeRequestModal = document.getElementById('closeRequestModal');
    if(closeRequestModal) closeRequestModal.onclick = () => { document.getElementById('requestModal').style.display = 'none'; };

    const modalReportBtn = document.getElementById('modalReportBtn');
    if(modalReportBtn) modalReportBtn.onclick = () => {
        if (!currentModalItem) return;
    ...
    if(closeReportModal) closeReportModal.onclick = () => { document.getElementById('reportModal').style.display = 'none'; };
    ...
    if (proceedToUnlockBtn) {
        proceedToUnlockBtn.onclick = () => {
            const dontShow = document.getElementById('dontShowUnlockWarning');
            if (dontShow && dontShow.checked) {
                const until = new Date().getTime() + 7 * 24 * 60 * 60 * 1000;
                localStorage.setItem('auraflixSkipUnlockWarning', String(until));
            }
            const videoContainer = document.getElementById('unlockTutorialContainer');
            if (videoContainer) videoContainer.innerHTML = '';
            const modal = document.getElementById('unlockWarningModal');
...
function renderCustomCategories() {
    const allContent = [...window.moviesList, ...window.seriesList];
    if (typeof window.customCategories !== 'undefined') {
        renderCategoryGroup(window.customCategories, 'customCategoriesContainer', allContent);
    }
    if (typeof window.movieCategories !== 'undefined') {
        renderCategoryGroup(window.movieCategories, 'movieCategoriesContainer', window.moviesList, 'view-movies', 'allMoviesGrid');
    }
    if (typeof window.seriesCategories !== 'undefined') {
        renderCategoryGroup(window.seriesCategories, 'seriesCategoriesContainer', window.seriesList, 'view-series', 'allSeriesGrid');
    }
}

function renderCategoryGroup(categoriesData, containerId, contentList, parentId = null, insertBeforeId = null) {
    ...
    categoriesData.forEach(category => {
        ...
        category.ids.forEach(id => {
            const item = contentList.find(i => String(i.id) === String(id));
        ...
        if (categoryItems.length > 0) {
            ...
            if (isHome) {
                htmlOutput += categoryItems.map((item, idx) => createItemHTML(item, idx)).join('');
            } else {
                htmlOutput += categoryItems.map((item, idx) => createItemHTML(item, idx, 'category-horizontal')).join('');
            }
            htmlOutput += `</div>`;
        }
    });
...
window.addEventListener('popstate', (event) => {
    ...
    if (!event.state || !event.state.modal) {
        ...
        const idsToHide = ['termsModal','settingsModal','supportModal','filterModal','changeAvatarModal','seasonSelectorModal','requestModal','reportModal','shareModal'];
