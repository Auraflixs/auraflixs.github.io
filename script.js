...
// SERVIDORES QUE DAN FALSOS POSITIVOS (Responden OK aunque el video esté borrado)
const TRICKY_HOSTS = [
    'dropload', 'streamtape', 'netu', 'uqload', 'dood', 'waaw', 'vidoza'
];
...
window.addEventListener('load', () => {
    // Si estamos usando el archivo de enlaces, verificamos que cargó
    if(typeof AURA_LINKS === 'undefined') console.log("Aviso: links.js no cargado o no existe.");
...
function setupEventListeners() {
    ...
    document.getElementById('btnBroadcast').onclick = () => {
        if (typeof AURA_LINKS !== 'undefined' && AURA_LINKS.telegram) {
            ...
            console.log("Error: links.js no cargado o sin propiedad telegram");
        }
    };
    ...
    // --- INTEGRACIÓN LINKS.JS COMPARTIR ---
...
// ==========================================
// SISTEMA DE DIAGNÓSTICO (DETECCIÓN MEJORADA)
// ==========================================
...
// LISTA DE SERVIDORES TRAMPOSOS (Responden OK aunque estén borrados)
const TRICKY_HOSTS = ['dropload', 'streamtape', 'netu', 'uqload', 'dood', 'waaw', 'vidoza'];
...
async function checkLink(item, logContainer) {
    ...
    // VERIFICACIÓN DE DOMINIOS TRAMPOSOS
    const isTricky = TRICKY_HOSTS.some(host => url.includes(host));
    ...
    try {
        ...
        // Si es un servidor tramposo y "respondió", avisamos que podría ser falso
        if (isTricky) {
            logToConsole(logContainer, item.title, "Online (Posible falso positivo - Revisar Manualmente)", "warning");
        } 
