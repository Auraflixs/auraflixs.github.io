...
function openSeasonSelectorModal() {
    ...
    currentModalItem.seasons.forEach((seasonObj, index) => {
        ...
        itemBtn.onclick = () => {
            ...
            // Renderizamos la nueva temporada desde el ep 0 (Calcula acceso por episodio dentro)
...
function setPlayerVideo(url, overlayText = null, isBlocked = false) {
    ...
    if (isBlocked) {
        ...
        if (window.UNLOCK_LINK_STATUS === 'on' || typeof window.UNLOCK_LINK_STATUS === 'undefined') {
            buttonsHtml += `<button onclick="showUnlockWarning()" class="vip-redirect-btn" style="margin-bottom: 12px; width: 100%;"><i class="fas fa-unlock"></i> Desbloquear Contenido</button>`;
        }
...
function closeModalInternal() {
    ...
    // Al cerrar limpiamos la variable para que no siga desbloqueado si abre otro
    window.unlockedItemId = null;
    window.unlockedSeason = null;
    window.unlockedEpisode = null;
...
function setupEventListeners() {
    ...
    // MODAL DE ADVERTENCIA PARA DESBLOQUEAR
