...
function initApp() {
    ...
    setupHero(); // Esto ahora leerá tus IDs manuales
    setupEventListeners();
...
function setupHero() {
    // 1. Combinamos todo el contenido disponible
    const allContent = [...moviesListInternal, ...seriesListInternal];

    // 2. Verificamos si pusiste IDs manualmente en el HTML (window.heroIds)
    if (window.heroIds && window.heroIds.length > 0) {
        // Buscamos esos IDs específicos
        featuredList = window.heroIds.map(id => 
            allContent.find(item => String(item.id) === String(id))
        ).filter(item => item !== undefined); // Filtramos si alguno no se encontró
    } else {
        // Si no pusiste lista, usamos la lógica antigua (aleatoria featured)
