...
// --- CONFIGURACIÓN SHRINK.PE Y BYPASS ---
...
function hasValidShortenerBypass() {
    ...
    return (Date.now() - parseInt(lastPass, 10)) < 86400000;
}
// ----------------------------------------
    ...
    if (urlParams.has('movie') || urlParams.has('serie')) {
        ...
        if (urlParams.has('movie')) {
            ...
            if (parts.includes('vip')) {  
                ...
                // Validamos que venga de shrink.pe
                const cameFromShortener = referrer.includes('shrink.pe');  
                const hasValidSessionToken = (returnToken === String(id));  

                // Se evalúa el ticket + validación (o si ya tiene bypass previo)
                if ((ticketTime && (now - parseInt(ticketTime)) < 1800000 && (hasValidSessionToken || cameFromShortener)) || hasValidShortenerBypass()) {  
                    setUnlockedForTicket(String(id), 24);  
                    localStorage.removeItem('auraflix_tkt_' + id);  
                    localStorage.removeItem('auraflix_return_token');  
                    localStorage.setItem('last_shortener_pass', Date.now()); // Se activa el bypass de 24h
                    window.unlockedItemId = id;  
                }  
            }  
            // Esta línea es la que abre el modal de la película automáticamente
            ...
            // Esta línea abre el modal de la serie automáticamente
        ...
        btnCache.onclick = () => {
            showCustomConfirm(
                ...
                () => {
                    ...
                    localStorage.removeItem('last_shortener_pass'); // BORRAMOS BYPASS AL LIMPIAR
