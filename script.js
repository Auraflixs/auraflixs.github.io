...
// ==========================================
// SISTEMA DE NOTIFICACIONES (BURBUJAS / TOAST)
// ==========================================
function showToast(message) {
    let toast = document.getElementById('auraflix-toast');
    if (!toast) {
        toast = document.createElement('div');
        toast.id = 'auraflix-toast';
        toast.style.position = 'fixed';
        toast.style.bottom = '90px'; // Un poco más arriba para no tapar la nav
        toast.style.left = '50%';
        toast.style.transform = 'translateX(-50%)';
        toast.style.backgroundColor = 'rgba(138, 43, 226, 0.95)'; // Morado cristal
...
function updateNavIndicator() {
    ...
    if (activeItem && indicator) {
        ...
        // Hacemos que la burbuja sea del tamaño perfecto para cubrir el ícono (aprox 60-70px)
        const bubbleWidth = Math.min(itemWidth * 0.75, 65); 
        indicator.style.width = `${bubbleWidth}px`; 
        
        // Posición central exacta adaptada al nuevo contenedor flotante
        const centerX = activeItem.offsetLeft + (itemWidth / 2);
        
        // Movemos la burbuja a ese centro usando transform (esto activa la transición elástica)
...
// ==========================================
// SEGURIDAD: VERIFICACIÓN DE LINKVERTISE + TICKET
// ==========================================
function checkDeepLinks() {
    ...
    if (urlParams.has('movie')) {
        ...
        if (parts.includes('vip')) {
            ...
            // Verifica que provenga estrictamente de linkvertise u otros acortadores
            if (!ref.includes('linkvertise.com') && !ref.includes('link-center.net') && !ref.includes('link-target.net') && !ref.includes('link-hub.net')) {
                showToast("Error V4: Acceso no autorizado");
                ...
                if (ticketTime && (now - parseInt(ticketTime)) < 1800000) {
                    ...
                    showToast("Error V4 (Sesión expirada)");
                    window.history.replaceState({}, '', window.location.href.replace('-vip', ''));
                }
            }
        }
...
function openModal(id, typeHint, fromDeepLink = false) {
    ...
    if (isSeries) {
        ...
        // SOLO GUARDAR EN HISTORIAL SI ESTÁ DESBLOQUEADA
        if (hasAccess) {
...
function renderEpisodes(season, serieItem, autoPlayIndex = -1) {
    ...
    if (autoPlayIndex >= 0 && season.episodes[autoPlayIndex]) {
        ...
        // SOLO GUARDAR EN HISTORIAL SI EL CAPÍTULO ESTÁ DESBLOQUEADO
        if (hasAccess) {
...
function setupEventListeners() {
    ...
    if (btnInstall) {
        btnInstall.onclick = () => {
            if(typeof AURA_LINKS !== 'undefined' && AURA_LINKS.appDownload) {
                ...
                showToast("Enlace de actualización no disponible.");
            }
        };
    }
    ...
    document.getElementById('btnCache').onclick = () => {
        if(confirm("¿Estás seguro de borrar tu historial y favoritos?")) {
