...
// 2. Borrar Caché (Favoritos y Recientes)
document.getElementById('btnCache').onclick = () => {
    if(confirm("¿Borrar historial y favoritos guardados?")) {
        ...
        myFavorites = [];
        renderMyList();
        renderContinueWatching(); // Si existiera la funcion para refrescar home
        alert("Caché borrada.");
    }
};
...
// 4. Cerrar Sesión (Borra TODO)
document.getElementById('logoutBtn').addEventListener('click', () => {
    if(confirm("¿Cerrar sesión y borrar todos los datos?")) {
        localStorage.clear(); // Borra user, favs, history
        location.reload();
    }
});
