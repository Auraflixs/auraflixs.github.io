...
/* ---------- checkDeepLinks: ahora exige URL con -vip y referrer ouo.io para marcar desbloqueo (24h) ---------- */
function checkDeepLinks() {
    ...
    if (urlParams.has('movie')) {
        ...
        if (parts.includes('vip')) {
            ...
            // SOLO marcar desbloqueado si la llegada tiene -vip Y viene de ouo.io Y existe ticket válido
            if (ticketTime && (now - parseInt(ticketTime)) < 1800000 && cameFromOuo) {
                // marcar desbloqueo por 24 horas
                setUnlockedForTicket(String(id), 24);
                localStorage.removeItem('auraflix_tkt_' + id);
                window.unlockedItemId = id; 
            }
        }
        // abrimos modal siempre con fromDeepLink=true para evitar pushState duplicado
...
/* --- NUEVO: comportamiento de desbloqueo automático usando OUO --- */
...
window.executeUnlock = function() {
    ...
    // guardamos ticket temporal (30 min) para validar al volver con -vip desde OUO
    ...
    if (dest) {
        ...
        // NO marcar como desbloqueado aquí: la marca la hará checkDeepLinks al volver con -vip + referrer=ouo.io
        window.location.assign(redirectTo);
    } else {
        showToast("No se pudo generar el enlace de desbloqueo.");
    }
};
/* --- fin desbloqueo automático --- */
...
/* --------- closeModalInternal: evita regresar al acortador si viniste de OUO o URL contiene -vip --------- */
function closeModalInternal() {
    ...
    // Si la URL actual contiene '-vip' (llegada desde el acortador) o el referrer fue ouo.io,
    // reemplazamos el historial por la URL base y llevamos al usuario al home (no volvemos al acortador).
    const cameFromOuo = (document.referrer && document.referrer.includes('ouo.io'));
...
/* --------- fin closeModalInternal modificado --------- */
...
function setupEventListeners() {
    ...
    // Al cerrar modal con la flecha, usamos lógica segura para no volver al acortador
    ...
    // ---------- SHARE en perfil ----------
    const btnShareProfile = document.getElementById('btnShare');
    if (btnShareProfile) {
    ...
    const btnShareAppVersion = document.getElementById('btnShareAppVersion');
    if (btnShareAppVersion) {
        btnShareAppVersion.onclick = () => {
            const appUrl = (typeof AURA_LINKS !== 'undefined' && AURA_LINKS.appDownload) ? AURA_LINKS.appDownload : (window.location.origin + window.location.pathname);
            shareGeneric("Descarga Auraflix", appUrl);
        };
    }
    const btnShareWebVersion = document.getElementById('btnShareWebVersion');
    if (btnShareWebVersion) {
        btnShareWebVersion.onclick = () => {
            const webUrl = (typeof AURA_LINKS !== 'undefined' && AURA_LINKS.webPage) ? AURA_LINKS.webPage : (window.location.origin + window.location.pathname);
            shareGeneric("Visita Auraflix", webUrl);
        };
    }
    // --------------------------------------
    ...
    document.getElementById('btnCache').onclick = () => {
        showCustomConfirm(
            ...
            () => {
                ...
                // borrar todos los auraflix_unlocked_until_*
    ...
    /* ===== CORRECCIÓN: controlar botones del unlockWarningModal ===== */
    ...
    if (proceedToUnlockBtn) {
        proceedToUnlockBtn.onclick = () => {
            ...
            if (dontShow && dontShow.checked) {
                const until = new Date().getTime() + 7 * 24 * 60 * 60 * 1000; // 7 días
            ...
            const modal = document.getElementById('unlockWarningModal');
            if (modal) modal.style.display = 'none';
            // luego ejecutamos el unlock (redirige al acortador OUO)
            executeUnlock();
        };
    }
    /* ===== fin corrección ===== */
...
function renderCustomCategories() {
    ...
    if (!container || typeof window.customCategories === 'undefined') return;
    const allContent = [...window.moviesList, ...window.seriesList];
    let htmlOutput = '';
