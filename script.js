...
function openModal(id, typeHint, fromDeepLink = false) {
    ...
    // 🔥 MODAL VISIBLE PRIMERO para evitar el bloqueo 0x0
    modal.style.display = 'flex';
...
// ==========================================
// FUNCIÓN DEL REPRODUCTOR CORREGIDA
// ==========================================
function setPlayerVideo(url, overlayText = null, isBlocked = false) {
    ...
    playerDiv.innerHTML = '<div style="display:flex; justify-content:center; align-items:center; height:100%; width:100%; color:#8A2BE2;"><i class="fas fa-spinner fa-spin fa-2x"></i></div>';

    // 2. Retrasar carga para evitar condición de carrera (tamaño 0x0)
    setTimeout(() => {
        playerDiv.innerHTML = ''; 
        if (!url) {
        ...
        if (isDirectFile) {
            ...
            if(isBlocked) {
                 ...
                 // 3. Sandox estricto: Bloquea ventanas emergentes y redirecciones de página completa
        ...
        // El overlay de texto solo se agrega para archivos directos, no iframes
