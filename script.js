...
function initApp() {
    renderHomeView();
    // CAMBIO IMPORTANTE: Ponemos las Series PRIMERO y las Películas AL FINAL en la lista combinada.
    // Esto asegura que al hacer .reverse(), las películas (que están al final) queden PRIMERAS.
    const allContent = [...seriesListInternal, ...moviesListInternal];
    ...
    if (window.allContentSequence && window.allContentSequence.length > 0) {
        ...
        // Al hacer reverse(), los últimos elementos del array (pelis nuevas) quedan de primeros.
...
// *** FUNCIÓN MODAL CON LÓGICA DE GÉNEROS 4 + ... ***
function openModal(id, typeHint) {
    ...
    // --- LÓGICA DE GÉNEROS (MÁXIMO 4) ---
    let genreText = "";
    if (item.genres && Array.isArray(item.genres)) {
        // Tomar máximo 4
        let displayGenres = item.genres.slice(0, 4);
        // Unir con barra
        genreText = displayGenres.join(' / ');
        // Agregar "..." si hay más de 4
