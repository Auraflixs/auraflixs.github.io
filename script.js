...
/* ---------- checkDeepLinks: ahora valida que el referrer provenga de ouo.io para desbloquear ---------- */
function checkDeepLinks() {
    ...
    if (urlParams.has('movie')) {
        ...
        if (parts.includes('vip')) {
            ...
            // sólo desbloquea si hay ticket válido y el referrer es ouo.io
            if (ticketTime && (now - parseInt(ticketTime)) < 1800000 && cameFromOuo) {
                ...
                // si no viene de ouo, no desbloquea: opcionalmente borra el ticket (para seguridad)
                // localStorage.removeItem('auraflix_tkt_' + id);
            }
        }
        if (window.location.href.includes('-vip')) {
            // reescribimos la url para quitar rastros si queremos
        ...
        if (parts.includes('vip')) {
            ...
            if (ticketTime && (now - parseInt(ticketTime)) < 1800000 && cameFromOuo) {
                ...
                // no desbloquear si no viene de ouo
                // localStorage.removeItem('auraflix_tkt_' + ticketKey);
            }
        }
        if (window.location.href.includes('-vip')) {
...
/* --------- closeModalInternal: ahora evita regresar al acortador si viniste de OUO o URL tiene -vip --------- */
...
function setupEventListeners() {
    ...
    document.getElementById('closeModal').addEventListener('click', () => {
        closeModalInternal();
        // Si viniste de OUO o la URL contiene -vip, ya se hizo replaceState dentro de closeModalInternal
        const cameFromOuo = (document.referrer && document.referrer.includes('ouo.io'));
        const isVipUrl = window.location.href.includes('-vip');
        if (!cameFromOuo && !isVipUrl) {
            // comportamiento por defecto para navegación normal
            window.history.back();
        } else {
            // ya reemplazamos el estado y llevamos al home en closeModalInternal
            // (no hacemos history.back para evitar volver al acortador)
        }
    });
