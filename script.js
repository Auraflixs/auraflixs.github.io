...
function openModal(id, typeHint, fromDeepLink = false) {
    let isSearch = document.getElementById('searchOverlay').style.display === 'block';
    let isHist = document.getElementById('historyOverlay').style.display === 'block';

    if (!document.body.classList.contains('modal-open') && !fromDeepLink) {
        window.history.pushState({ view: currentView, modal: true, search: isSearch, history: isHist }, '');
    }
...
function closeModalInternal() {
    ...
    window.history.replaceState({ view: currentView, modal: false, search: isSearch, history: isHist }, '', baseUrl);
}
