...
function showToast(message) {
    const toast = document.getElementById('auraflix-toast');
    if (!toast) return;
...
// ==========================================
// LÓGICA LIQUID GLASS INDICATOR (BURBUJA) 
// ==========================================
...
// ==========================================
// SEGURIDAD: VERIFICACIÓN VIP Y APK
// ==========================================
    ...
    if (urlParams.has('movie')) {
        ...
        if (parts.includes('vip')) {
            ...
            if (ticketTime && (now - parseInt(ticketTime)) < 1800000) {
                ...
                showToast("Enlace de desbloqueo expirado o inválido.");
                window.history.replaceState({}, '', window.location.href.replace('-vip', ''));
            }
        }
...
function handleImageUpload(e) {
    ...
    reader.onload = function(event) {
        ...
        showToast("Imagen lista. Presiona Guardar para aplicar.");
    };
...
// ==========================================
// SISTEMA DE ADMIN GLOBAL
// ==========================================
window.isUserAdmin = function() {
    return currentUser && currentUser.name === 'L-ad';
};
...
function loadUserDataInUI() {
    ...
    if (typeof checkAdminPrivileges === 'function') checkAdminPrivileges();
...
function createItemHTML(item, index = 100) {
    ...
    let badgeHTML = '';
    if (item.badge) {
        badgeHTML = `<span class="new-content-badge">${item.badge}</span>`;
    }
...
function openModal(id, typeHint, fromDeepLink = false) {
    ...
    const isGlobalAdmin = window.isUserAdmin ? window.isUserAdmin() : false; 
    ...
    if (isSeries) {
        ...
        // VERIFICAMOS SI LA PELÍCULA REQUIERE DESBLOQUEO O SI ES GRATIS
        const requiresUnlock = item.unlockLink && item.unlockLink.trim() !== '';
        ...
        // El usuario tiene acceso si: Es Admin, es VIP, ya lo desbloqueó, o si la peli NO requiere desbloqueo (es gratis)
        const hasAccess = !requiresUnlock || isGlobalAdmin || isVipGlobal || isUnlockedViaLink || isUnlockedViaMemory;
        
        if (shareBtn) shareBtn.style.display = hasAccess ? 'flex' : 'none';

        // SOLO GUARDAR EN HISTORIAL SI TIENE ACCESO (ESTÁ DESBLOQUEADA O ES GRATIS)
        if (hasAccess) {
...
function renderEpisodes(season, serieItem, autoPlayIndex = -1) {
    ...
    const checkAccess = (ep) => {
        const requiresUnlock = ep.unlockLink && ep.unlockLink.trim() !== '';
        if (!requiresUnlock || isGlobalAdmin || isVipGlobal) return true;
    ...
    if (autoPlayIndex >= 0 && season.episodes[autoPlayIndex]) {
        ...
        // SOLO GUARDAR EN HISTORIAL SI EL CAPÍTULO TIENE ACCESO
        if (hasAccess) {
...
function renderPopularSearches() {
    ...
    const popTitle = document.getElementById('popularSearchTitle'); 
    
    if (!container || !window.busquedasPopulares) return;

    if (input.value.trim() === '') {
        container.style.display = 'flex';
        if(popTitle) popTitle.style.display = 'block';
        ...
        if(popTitle) popTitle.style.display = 'none';
    }
}
...
// ==========================================
// FUNCIÓN PARA EL NUEVO MODAL DE CONFIRMACIÓN
// ==========================================
function showCustomConfirm(title, message, onAcceptCallback) {
    ...
    // Clonamos los botones para borrar eventos anteriores y evitar clics duplicados
...
function setupEventListeners() {
    ...
    // BOTÓN DE CANJEAR CÓDIGO (ABRIR Y CERRAR MODAL)
    ...
    const closeRedeem = document.getElementById('closeRedeemModal');
    if (closeRedeem) {
        closeRedeem.onclick = () => document.getElementById('redeemModal').style.display = 'none';
    }
    
    // LOGICA PREPARADA PARA EL CANJEO DEL CÓDIGO
    const btnApplyRedeem = document.getElementById('btnApplyRedeem');
    if (btnApplyRedeem) {
        btnApplyRedeem.onclick = () => {
            const codeInput = document.getElementById('redeemCodeInput').value.trim();
            if(!codeInput) {
                showToast("Por favor ingresa un código.");
                return;
            }
            // Aquí se conectará en el futuro con tu codevip.js
            if(typeof window.procesarCodigoVip === 'function') {
                window.procesarCodigoVip(codeInput);
            } else {
                showToast("El sistema de códigos se activará pronto.");
    ...
    // APLICAMOS LA CONFIRMACIÓN CON ESTILO OSCURO A BORRAR CACHÉ
    document.getElementById('btnCache').onclick = () => {
        document.getElementById('settingsModal').style.display = 'none'; // Cerramos ajustes primero
        showCustomConfirm("Borrar Caché", "¿Estás seguro de borrar tu historial y favoritos de Auraflix?", () => {
    ...
    // CERRAR SESIÓN CON EL NUEVO MODAL OSCURO
    document.getElementById('logoutBtn').addEventListener('click', () => {
        showCustomConfirm("Cerrar Sesión", "¿Estás seguro de que deseas salir de tu cuenta?", () => {
            localStorage.clear(); 
            location.reload();
        });
    });
...
window.addEventListener('popstate', (event) => {
    ...
    if (!event.state || !event.state.modal) {
        ...
        document.getElementById('redeemModal').style.display = 'none'; // También se oculta con retroceso
