...
window.__auraWatchdogTimer = null;
window.__auraPlayerMessageInstalled = false;

// ─────────────────────────────────────────────────────────────
// [CAMBIO 1] NUEVO ACORTADOR: shrink.pe
// Se reemplaza OUO_PREFIX por SHRINK_PREFIX con el formato exacto
// proporcionado. El parámetro &url= recibirá la URL de destino
// codificada con encodeURIComponent.
// ─────────────────────────────────────────────────────────────
const SHRINK_PREFIX = 'https://shrink.pe/st?api=8bbf66d1d4e8c19d60214d2ad601324c36921a6d&url=';

// ─────────────────────────────────────────────────────────────
// [CAMBIO 3 - PARTE A] BYPASS DE 24 HORAS
// Esta función verifica si el usuario ya pasó por el acortador
// en las últimas 24 horas (86.400.000 ms).
// Devuelve TRUE si tiene bypass activo → acceso directo.
// Devuelve FALSE si debe pasar por el acortador.
// ─────────────────────────────────────────────────────────────
function hasValidShortenerBypass() {
    const lastPass = localStorage.getItem('last_shortener_pass');
    if (!lastPass) return false;
    const elapsed = Date.now() - parseInt(lastPass, 10);
    return elapsed < 86400000; // 24 horas en milisegundos
}
...
function installPlayerMessageHandler() {
    ...
    window.addEventListener('message', (e) => {
        ...
        if (e.origin !== currentPlayerOrigin && e.origin !== 'null' && currentPlayerOrigin !== '*') { }
        let data = e.data;
        ...
        const isReadyEvent = data.playerStatus === 'Ready' || data.playerStatus === 'Listo' || data.event === 'ready' || data.event === 'loaded' || data.event === 'loadedmetadata';
        ...
        if (isReadyEvent || currentTime !== undefined || duration !== undefined) {
            window.__auraPlayerIsReady = true;
        }

        if (duration > 0) window.__auraPlayerHasMetadata = true;

        if (currentPlayerProgressKey && !window.__auraHasSeeked && window.__auraPlayerIsReady) {
            const savedTimeStr = localStorage.getItem(currentPlayerProgressKey);
            const savedTime = savedTimeStr ? parseFloat(savedTimeStr) : 0;
            if (savedTime > 5) {
                if (window.__auraPlayerHasMetadata || (currentTime !== undefined && currentTime >= 0)) {
                    if (currentTime === undefined || Math.abs(currentTime - savedTime) > 5) {
                        if (!window.__seekingNow) {
                            window.__seekingNow = true;
                            setTimeout(() => {
                                if (currentPlayerIframe && currentPlayerIframe.contentWindow) {
                                    currentPlayerIframe.contentWindow.postMessage({ command: 'seek', value: savedTime }, currentPlayerOrigin);
                                    setTimeout(() => {
                                        if (currentPlayerIframe && currentPlayerIframe.contentWindow) {
                                            currentPlayerIframe.contentWindow.postMessage({ command: 'play' }, currentPlayerOrigin);
                                            window.__auraHasSeeked = true;
                                            window.__seekingNow = false;
                                        }
                                    }, 600);
                                }
                            }, 400);
        ...
        if (window.__auraHasSeeked && typeof currentTime === 'number' && currentPlayerProgressKey) {
...
function initLogoTyping() {
    ...
    function type() {
        ...
        if (charIndex > 0 || isDeleting) {
            logoContainer.classList.add('typing');
        } else {
            logoContainer.classList.remove('typing');
        }
        ...
        if (!isDeleting && charIndex < textToType.length) {
            ...
            setTimeout(type, isDeleting ? 3000 : 500);
        }
    }
    ...
    if (urlParams.get('page') === 'solicitudes') {
        ...
        if (reqModal) {
            ...
            setTimeout(() => {
                ...
                if (reqM) reqM.style.display = 'flex';
            }, 500);
        }
    }
    ...
    if (urlParams.has('movie') || urlParams.has('serie')) {
        ...
        if (urlParams.has('movie')) {
            ...
            if (parts.includes('vip')) {
                ...
                // ─────────────────────────────────────────────────────────
                // [CAMBIO 2] VERIFICADOR ACTUALIZADO: shrink.pe
                // Se reemplaza la comprobación de ouo.io / ouo.press
                // por shrink.pe para validar que el usuario viene
                // correctamente del nuevo acortador.
                // ─────────────────────────────────────────────────────────
                const cameFromShortener = referrer.includes('shrink.pe');
                const hasValidSessionToken = (returnToken === String(id));

                if (ticketTime && (now - parseInt(ticketTime)) < 1800000 && (hasValidSessionToken || cameFromShortener)) {
                    setUnlockedForTicket(String(id), 24);
                    localStorage.removeItem('auraflix_tkt_' + id);
                    localStorage.removeItem('auraflix_return_token');

                    // ─────────────────────────────────────────────────────
                    // [CAMBIO 3 - PARTE B] GUARDAR TIMESTAMP DEL BYPASS
                    // Cuando el usuario completa el acortador exitosamente
                    // guardamos el momento actual en last_shortener_pass.
                    // Esto activa el bypass de 24 horas.
                    // ─────────────────────────────────────────────────────
                    localStorage.setItem('last_shortener_pass', Date.now());
            ...
            if (parts.includes('vip')) {
                ...
                // ─────────────────────────────────────────────────────────
                // [CAMBIO 2] VERIFICADOR ACTUALIZADO: shrink.pe (series)
                // Misma lógica que en películas: se valida shrink.pe
                // en lugar de ouo.io / ouo.press.
                // ─────────────────────────────────────────────────────────
                const cameFromShortener = referrer.includes('shrink.pe');
                ...
                if (ticketTime && (now - parseInt(ticketTime)) < 1800000 && (hasValidSessionToken || cameFromShortener)) {
                    ...
                    // ─────────────────────────────────────────────────────
                    // [CAMBIO 3 - PARTE B] GUARDAR TIMESTAMP DEL BYPASS
                    // También se guarda el timestamp para series,
                    // activando el bypass de 24 horas global.
                    // ─────────────────────────────────────────────────────
                    localStorage.setItem('last_shortener_pass', Date.now());
...
function checkAdminPrivileges() {
    ...
    if (allowedUsers.includes(currentUser.name) || window.isAdminViaUrl) {
...
function openModal(id, typeHint, fromDeepLink = false) {
    ...
    if (!fromDeepLink) {
        ...
        if (!isSeries) {
            cleanUrl += `?movie=${item.id}-${item.year || '2025'}`;
        }
    ...
    if (shareBtn) {
        shareBtn.onclick = () => {
            ...
            if (navigator.share) {
                navigator.share({
                    title: `Ver ${item.title} en Auraflix's`,
    ...
    const isVipGlobal = window.isAdminViaUrl || (typeof isUserVip === 'function' ? isUserVip() : false);
    ...
    if (isSeries) {
        ...
        const hasAccess = window.isAdminViaUrl || ((window.VIP_SYSTEM_STATUS === 'off' && window.UNLOCK_LINK_STATUS === 'off')
            ? true
...
function renderEpisodes(season, serieItem, autoPlayIndex = -1) {
    ...
    const checkAccess = (ep) => {
        if (window.isAdminViaUrl) return true;
...
// ─────────────────────────────────────────────────────────────
// [CAMBIO 3 - PARTE C] BYPASS EN executeUnlock
// Antes de redirigir al acortador, verificamos si el usuario
// ya tiene un bypass válido de 24 horas (hasValidShortenerBypass).
// Si el bypass está activo → acceso directo sin acortador.
// Si no → se construye la URL con SHRINK_PREFIX (cambio 1)
//          y se redirige al acortador shrink.pe.
// ─────────────────────────────────────────────────────────────
window.executeUnlock = function() {
    ...
    // ── LÓGICA DE BYPASS: verificar si han pasado < 24 horas ──
    if (hasValidShortenerBypass()) {
        // El usuario ya pasó por el acortador recientemente.
        // Desbloqueamos directamente sin redirigir al acortador.
        setUnlockedForTicket(ticketKey, 24);
        // Recargamos el modal para que refleje el acceso desbloqueado.
        openModal(currentModalItem.id, currentModalItem.seasons ? 'series' : 'movies', true);
        showToast("✅ Acceso directo activo (bypass 24h)");
        return; // <-- Salimos aquí, no vamos al acortador
    }

    // Sin bypass: preparamos y redirigimos al acortador shrink.pe
    ...
    if (dest) {
        // [CAMBIO 1] Se usa SHRINK_PREFIX en lugar del anterior OUO_PREFIX
        const redirectTo = SHRINK_PREFIX + encodeURIComponent(dest);
...
window.showUnlockWarning = function() {
    ...
    // Si el usuario tiene bypass activo, ejecutamos directamente sin mostrar el aviso
    if (hasValidShortenerBypass()) {
        executeUnlock();
        return;
    }
...
function setPlayerVideo(url, overlayText = null, isBlocked = false) {
    ...
    window.__auraCurrentPlayerTime = undefined;
    window.__auraPlayerIsReady = false;
    window.__auraPlayerHasMetadata = false;
    window.__seekingNow = false;
    ...
    if (window.__auraWatchdogTimer) {
        clearTimeout(window.__auraWatchdogTimer);
        window.__auraWatchdogTimer = null;
    }
    ...
    if (isBlocked) {
        ...
        if (window.UNLOCK_LINK_STATUS === 'on' || typeof window.UNLOCK_LINK_STATUS === 'undefined') {
            buttonsHtml += '<button onclick="showUnlockWarning()" class="vip-redirect-btn" style="background: linear-gradient(45deg, #8A2BE2, #A66BFF); margin-bottom: 12px; width: 100%; box-shadow: 0 0 10px rgba(138, 43, 226, 0.5);"><i class="fas fa-unlock"></i> Ver anuncio</button>';
        }
        ...
        blockerDiv.innerHTML = '<i class="fas fa-lock vip-lock-icon"></i><p class="vip-overlay-text" style="margin-bottom: 25px;">Necesitas ver un anuncio antes de ver la película</p><div style="display: flex; flex-direction: column; width: 85%; max-width: 250px;">' + buttonsHtml + '</div>';
    ...
    window.__auraWatchdogTimer = setTimeout(() => {
        if (!window.__auraPlayerIsReady && currentPlayerIframe) {
            currentPlayerIframe.src = iframe.src;
        }
    }, 8000);
}
...
function setupEventListeners() {
    ...
    if (closeRedeemModal) closeRedeemModal.onclick = () => document.getElementById('redeemModal').style.display = 'none';
        ...
        btnCache.onclick = () => {
            showCustomConfirm(
                ...
                () => {
                    ...
                    localStorage.removeItem('last_shortener_pass'); // También limpiamos el bypass
