...
const css = `
  ...
  .row.horizontal-scroll.category-horizontal .item { min-width: auto !important; width: calc(33.333% - 7px); flex: 0 0 auto; margin-bottom: 15px; }
  ...
  .row.horizontal-scroll.category-horizontal .item img { width: 100%; aspect-ratio: 2/3; height: auto; }
...
if (sizeMode === 'category-horizontal') {
    ...
    if (document && document.body && document.body.classList.contains('grid-small')) {
        inlineStyle = 'style="width: calc(25% - 8px); margin-bottom: 10px;"';
    } else {
        inlineStyle = 'style="width: calc(33.333% - 7px); margin-bottom: 15px;"';
    }
}
...
if (htmlOutput !== '' && !isHome) {
    htmlOutput += `<div style="width: 92%; height: 1px; background-color: #333; margin: 30px auto 15px auto;"></div>`;
}
