...
// Variables de sincronización seguras para Firebase
...
// Sincronizador mejorado e inmune a cargas asíncronas de Firebase
function checkDataReady() {
    if (window.appInitialized) return;
    
    // Esperamos a que Firebase Firestore complete la carga (o que los arrays tengan datos)
    if (window.catalogLoaded || window.moviesList.length > 0 || window.seriesList.length > 0) {
...
function setupEventListeners() {
    ...
    if(closeSearchBtn) closeSearchBtn.onclick = () => { wasSearchOpen = false; window.history.back(); };

    const closeHistoryBtn = document.getElementById('closeHistory');
    if(closeHistoryBtn) closeHistoryBtn.onclick = () => { wasHistoryOpen = false; window.history.back(); };
