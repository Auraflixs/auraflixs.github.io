...
let currentVideoIsTrailer = false;
...
function hasUserAccess() {
    ...
    if (!currentModalItem.seasons) {
        ...
        return isUnlockedViaMemory || isUnlockedViaLink;
    } else {
        ...
        return isUnlockedViaMemory || justUnlocked;
    }
}

function startFreeMinute() {
    if (currentVideoIsTrailer) return;
...
function installPlayerMessageHandler() {
    ...
    window.addEventListener('message', (e) => {
        ...
        if (isPlaying && !playerFirstPlayDetected) {
            ...
            if (!currentVideoIsTrailer && !hasUserAccess()) {
                startFreeMinute();
            }
        }
...
function openModal(id, typeHint, fromDeepLink = false) {
    ...
    if (isSeries) {
        ...
        currentVideoIsTrailer = false;
        setPlayerVideo(item.video, null);
    ...
    playerReadyTimeout = setTimeout(() => {
        if (playerLoadAttempts < maxPlayerLoadAttempts) {
            ...
            if (currentPlayerIframe) {
                ...
                setPlayerVideo(retryUrl, retryOverlay);
            }
        }
    }, 7000);
