...
(function() {
    var isAdmin = window.location.pathname.includes('/admin') ||
                  ...
                  sessionStorage.getItem('auraAdminMode') === 'true';
    if (isAdmin) sessionStorage.setItem('auraAdminMode', 'true');
...
function setPlayerVideo(url, overlayText, isBlocked) {
    ...
    if (savedTime && parseFloat(savedTime) > 0) {
        const timeSeconds = parseFloat(savedTime);
        if (finalUrl.includes('#')) {
            finalUrl += '&t=' + timeSeconds;
        } else {
            finalUrl += '#t=' + timeSeconds;
        }
    }
...
function setupEventListeners() {
    ...
    if(btnRequestContent) btnRequestContent.onclick = () => { window.location.href = 'https://auraflixs.github.io/solicitudes'; };
