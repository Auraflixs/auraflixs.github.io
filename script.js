...
function setupLatelyNew() {
    ...
    // Compatibilidad: LATELY_IDS entries pueden ser "id" o {id,img}
...
/* prefijo fijo del acortador */
const OUO_PREFIX = 'https://ouo.io/qs/U5nWPP5R?s=';

/* helper que construye la URL destino desbloqueada (con '-vip') */
function buildUnlockedDestUrlForCurrentItem() {
    ...
    if (!currentModalItem.seasons) {
        ...
        // serie: tomamos la temporada actual y episodio activo (o primer episodio)
...
/* executeUnlock ahora IGNORA unlockLink y usa OUO_PREFIX + encodeURIComponent(dest) */
window.executeUnlock = function() {
    if (!currentModalItem) return;
    // construimos una "clave" para memoria (igual lógica previa)
    let ticketKey = null;
    if (!currentModalItem.seasons) {
    ...
    // guardamos timestamp local para registrar intento de unlock
    localStorage.setItem('auraflix_tkt_' + ticketKey, new Date().getTime());

    // construimos la url destino desbloqueada y redirigimos a OUO
    const dest = buildUnlockedDestUrlForCurrentItem();
    if (dest) {
        const redirectTo = OUO_PREFIX + encodeURIComponent(dest);
        // añadimos a memoria desbloqueados (por si queremos marcar)
        if (!unlockedMemory.includes(ticketKey)) {
            unlockedMemory.push(ticketKey);
            localStorage.setItem('auraflixUnlockedMemory', JSON.stringify(unlockedMemory));
        }
...
/* función reutilizable para compartir / copiar enlaces */
...
function setupEventListeners() {
    ...
    if (btnShareAppVersion) {
        btnShareAppVersion.onclick = () => {
            // usa AURA_LINKS.appDownload si está definido, si no fallback a página
    ...
    if (btnShareWebVersion) {
        btnShareWebVersion.onclick = () => {
            // usa AURA_LINKS.webPage si está definido, si no fallback a location
