...
// ==========================================
// NUEVO: SISTEMA DE ADMIN GLOBAL
// ==========================================
window.isUserAdmin = function() {
    return currentUser && (currentUser.name === 'L-ad' || currentUser.name === 'Naho-ad');
};
...
function loadUserDataInUI() {
    ...
    if (window.isUserAdmin()) {
        if (badge) {
            ...
            badge.style.background = 'linear-gradient(45deg, #ff0044, #ff4d4d)';
            badge.style.color = 'white';
            badge.style.boxShadow = '0 0 10px rgba(255, 0, 68, 0.5)';
            badge.style.border = '1px solid #ff0044';
        }
    } else {
        if (badge) {
...
function openModal(id, typeHint, fromDeepLink = false) {
    ...
    const isGlobalAdmin = window.isUserAdmin ? window.isUserAdmin() : false; // NUEVO: Poder de ADMIN
    ...
    if (isSeries) {
        ...
        // ADMIN tiene acceso directo a todo.
        const hasAccess = isGlobalAdmin || isVipGlobal || isUnlockedViaLink || isUnlockedViaMemory;
...
function renderEpisodes(season, serieItem, autoPlayIndex = -1) {
    ...
    const checkAccess = (ep) => {
        if (isGlobalAdmin || isVipGlobal) return true;
...
function renderPopularSearches() {
    ...
    const popTitle = document.getElementById('popularSearchTitle'); // El p que dice "Búsqueda popular"
    
    if (!container || !window.busquedasPopulares) return;

    // Solo lo mostramos si el input está vacío
    if (input.value.trim() === '') {
        ...
        // Si hay texto, ocultamos las populares por completo
...
function setupEventListeners() {
    ...
    document.getElementById('topSearchBtn').onclick = (e) => {
        ...
        const input = document.getElementById('searchInput');
        input.value = ''; // Limpiamos el buscador al entrar
        input.focus();
        renderPopularSearches();
        document.getElementById('searchResults').innerHTML = ''; // Limpiamos resultados anteriores
    ...
    document.getElementById('searchInput').addEventListener('input', (e) => {
        ...
        // Ejecutar renderPopularSearches para que oculte los tags si hay texto
        renderPopularSearches(); 
