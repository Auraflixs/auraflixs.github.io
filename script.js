...
// --- Helpers para desbloqueo con expiración 24h ---
...
function injectImageStyles() {
    ...
    const css = `
        ...
        /* GRID / ROW CONSISTENT SIZING */
        .responsive-grid {
            display: grid;
            gap: var(--af-gap);
            grid-template-columns: repeat(var(--af-columns), 1fr);
            width: 100%;
        }
        .responsive-grid .item, .row.horizontal-scroll .item {
            border-radius: 6px;
            overflow: hidden;
            position: relative;
            aspect-ratio: 2 / 3; /* portada vertical estándar */
            min-height: 160px;
            display: flex;
            align-items: stretch;
        ...
        /* Horizontal scroll row item sizing (keeps same ratio but fixed width) */
        ...
        .row.horizontal-scroll .item {
            min-width: 140px;
            flex: 0 0 140px;
        }
        /* Make horizontal items slightly larger on wide screens */
        @media (min-width: 1200px) {
            .row.horizontal-scroll .item { min-width: 160px; flex: 0 0 160px; }
        }
        ...
        /* GlobalID badge for newly added */
        .global-badge {
            position: absolute;
            left: 8px;
            top: 8px;
...
function ensureGridContainers() {
    const moviesGrid = document.getElementById('allMoviesGrid');
    const seriesGrid = document.getElementById('allSeriesGrid');
    if (moviesGrid) moviesGrid.classList.add('responsive-grid');
    if (seriesGrid) seriesGrid.classList.add('responsive-grid');
}
...
function initApp() {
    ...
    // Construir lista "Recién agregadas" ordenada por globalid descendente (si existe)
    let newlyAdded = [];
    if (window.globalContent && Array.isArray(window.globalContent) && window.globalContent.length > 0) {
        newlyAdded = [...window.globalContent];
    } else {
        newlyAdded = [...(window.moviesList.originalOrder || []), ...(window.seriesList.originalOrder || [])];
    }

    // Sort por globalid descendente, usando fallback si no existe
    newlyAdded.sort((a, b) => {
        const ga = getGlobalId(a);
        const gb = getGlobalId(b);
        return gb - ga;
    });

    renderList('newlyAddedRow', newlyAdded.slice(0, 20));
    
    // Llamada a la nueva función de categorías
...
function getGlobalId(item) {
    if (!item) return 0;
    if (typeof item.globalid !== 'undefined' && item.globalid !== null) {
        const n = parseInt(item.globalid, 10);
        return isNaN(n) ? 0 : n;
    }
    // fallback: intentar parsear número del id (p10, s3, etc.) o del campo id numérico
...
function renderList(containerId, list) {
    ...
    if (!container) return;
    // Si el contenedor está marcado como grid responsive, aseguramos estilos
    if (container.classList && container.classList.contains('responsive-grid')) {
        // nothing extra for now, CSS hace el trabajo
    }
    container.innerHTML = list.map((item, i) => createItemHTML(item, i, containerId)).join('');
}
...
function createItemHTML(item, index = 100, containerContext = '') {
    const type = item.seasons ? 'series' : 'movies';
    const safeId = String(item.id).replace(/'/g, "\\'");
    const loadMode = index < 6 ? 'eager' : 'lazy';
    // badge con globalid solo en sección newlyAddedRow
    const isNewlyAddedContext = containerContext === 'newlyAddedRow';
    const globalIdVal = isNewlyAddedContext ? getGlobalId(item) : null;
    const globalBadgeHtml = (isNewlyAddedContext && globalIdVal) ? `<div class="global-badge">#${globalIdVal}</div>` : '';
    return `
        <div class="item img-loader" data-id="${safeId}" onclick="openModal('${safeId}', '${type}')">
            ${globalBadgeHtml}
            <img src="${item.image}" 
                 class="smooth-image" 
                 onload="this.parentElement.classList.remove('img-loader'); this.classList.add('loaded')" 
...
function setupEventListeners() {
    ...
    if (profilePageImg) profilePageImg.addEventListener('click', () => {
    ...
    if (closeReportModal) closeReportModal.onclick = () => {
...
function renderCategoryGroup(categoriesData, containerId, contentList, parentId = null, insertBeforeId = null) {
    ...
    if (!container && parentId && insertBeforeId) {
        ...
        if (parent && insertBeforeEl) {
            ...
            // Insertamos justo antes de la cuadrícula o los filtros
    ...
    categoriesData.forEach((category, idx) => {
        ...
        if (categoryItems.length > 0) {
            ...
            htmlOutput += `<div class="row horizontal-scroll">`;
            htmlOutput += categoryItems.map(item => createItemHTML(item)).join('');
            htmlOutput += `</div>`;
            // Divider visual entre categorías
            htmlOutput += `<div class="category-divider" aria-hidden="true"></div>`;
        }
    });
...
window.addEventListener('popstate', (event) => {
    ...
    if (!event.state || !event.state.modal) {
        ...
        const changeAv = document.getElementById('changeAvatarModal');
        if (changeAv) changeAv.style.display = 'none';
        const seasonSel = document.getElementById('seasonSelectorModal');
