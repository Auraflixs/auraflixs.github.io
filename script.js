...
function checkDeepLinks() {
    ...
    if (urlParams.has('movie')) {
        ...
        if (parts.includes('vip')) {
            window.sessionUnlockedVIP = true; 
        } else {
            window.sessionUnlockedNormal = true; 
        }
...
function openModal(id, typeHint) {
    ...
    // --- LÓGICA DEL BOTÓN DE COMPARTIR DINÁMICO ---
    if (shareBtn) {
        shareBtn.onclick = () => {
            ...
            // Verificamos si el usuario actual es VIP
            const isVipStatus = typeof isUserVip === 'function' ? isUserVip() : false;
            const vipStr = isVipStatus ? "-vip" : "";
            
            if (!isSeries) {
                // Formato Película: ?movie=id-vip-2025 o ?movie=id-2025
                shareUrl += `?movie=${item.id}${vipStr}-${item.year || currentYear}`;
            } else {
                // Formato Serie: ?serie=id-t1-e1-vip o ?serie=id-t1-e1
                ...
                shareUrl += `?serie=${item.id}-t${seasonObj.season}-e${epNumber}${vipStr}`;
            }
    ...
    // VERIFICACIÓN DE ESTADO VIP / ACORTADOR
    const isVipGlobal = typeof isUserVip === 'function' ? isUserVip() : false;
    // Si la sesión fue desbloqueada por URL, se otorga acceso temporal
    const isUnlockedViaLink = window.sessionUnlockedNormal || window.sessionUnlockedVIP;
    
    const hasAccess = isVipGlobal || isUnlockedViaLink;
    ...
    if (isSeries) {
        ...
        if (item.seasons && item.seasons.length > 0) {
            ...
            renderEpisodes(item.seasons[currentSeasonIndex], item, startEpIdx, hasAccess); 
        ...
        if (hasAccess) {
            ...
            setPlayerVideo(item.video, null, true); 
        }
...
function openSeasonSelectorModal() {
    ...
    currentModalItem.seasons.forEach((seasonObj, index) => {
        ...
        itemBtn.onclick = () => {
            ...
            // Re-evaluar acceso al cambiar de temporada
            const isVipGlobal = typeof isUserVip === 'function' ? isUserVip() : false;
            const isUnlockedViaLink = window.sessionUnlockedNormal || window.sessionUnlockedVIP;
            const hasAccess = isVipGlobal || isUnlockedViaLink;
            
            renderEpisodes(seasonObj, currentModalItem, 0, hasAccess); 
...
function renderEpisodes(season, serieItem, autoPlayIndex = -1, hasAccess = false) {
    ...
    if (autoPlayIndex >= 0 && season.episodes[autoPlayIndex]) {
        ...
        if(hasAccess) {
            ...
            setPlayerVideo(ep.video, `T${season.season}: Cap ${ep.episode}`, true); 
        }
    }
...
window.redirectToVipProfile = function() {
    ...
    // Si el usuario intentó acceder a algo no VIP, mostrar alerta sobre acortadores (Lógica configurable)
    if(window.vippLinks && currentModalItem && !currentModalItem.seasons) {
        const linkData = window.vippLinks.find(l => l.id == currentModalItem.id);
        if(linkData && linkData.link) {
            if(confirm("Puedes desbloquear esta película viendo un anuncio.\n\n¿Deseas ir al enlace?")) {
                window.open(linkData.link, '_blank');
                return;
            }
        }
    } else if (window.vipsLinks && currentModalItem && currentModalItem.seasons) {
         // Lógica similar para series (buscar en vips.js)
         // Dependerá de la estructura de vips.js
    }
...
function setPlayerVideo(url, overlayText = null, isBlocked = false) {
    ...
    if (isBlocked) {
        ...
        blockerDiv.innerHTML = `
            ...
            <p class="vip-overlay-text">Contenido Bloqueado</p>
            <button onclick="redirectToVipProfile()" class="vip-redirect-btn">Desbloquear / Ser VIP</button>
        `;
...
function closeModalInternal() {
    ...
    // Limpiar variables de sesión temporal al cerrar el modal
    window.sessionUnlockedNormal = false;
    window.sessionUnlockedVIP = false;
    
    // Limpiar URL para que al recargar no siga desbloqueado
    const baseUrl = window.location.href.split('?')[0];
