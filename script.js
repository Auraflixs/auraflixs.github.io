...
// ==========================================
// SEGURIDAD: VERIFICACIÓN DE TICKET + URL VIP (SILENCIOSO)
// ==========================================
function checkDeepLinks() {
    ...
    if (urlParams.has('movie')) {
        ...
        if (parts.includes('vip')) {
            ...
            // Si el ticket es válido y tiene menos de 30 minutos
            if (ticketTime && (now - parseInt(ticketTime)) < 1800000) {
                // Guardamos en la memoria permanente
                ...
                localStorage.removeItem('auraflix_tkt_' + id); // Consumir ticket
                // Limpiar el '-vip' de la URL de forma silenciosa
                window.history.replaceState({}, '', window.location.href.replace('-vip', ''));
            } else if (!unlockedMemory.includes(String(id))) {
                // FALLO SILENCIOSO: No hay alerta, solo limpiamos el link y dejamos que el modal se abra bloqueado
                window.history.replaceState({}, '', window.location.href.replace('-vip', ''));
            }
        }

        // Abre el modal persistente si recargaste la página
...
function openModal(id, typeHint, fromDeepLink = false) {
    ...
    if (isSeries) {
        ...
        if (item.seasons && item.seasons.length > 0) {
            ...
            // Reemplazar la URL por la limpia (sin vip) de forma silenciosa para que persista al recargar
            if (!fromDeepLink) {
...
function renderEpisodes(season, serieItem, autoPlayIndex = -1) {
    ...
    // Función auxiliar para chequear acceso del episodio (Memoria o Link)
...
window.executeUnlock = function() {
    if (currentModalItem) {
        ...
        if (shortLink) {
            // GENERAMOS EL TICKET PARA VALIDAR AL REGRESAR (30 MINUTOS)
...
function setupEventListeners() {
    ...
    document.getElementById('btnCache').onclick = () => {
        showCustomConfirm(
            "Borrar Caché",
            "¿Estás seguro de borrar tu historial de visualización y favoritos?",
            () => {
                localStorage.removeItem('continueWatching');
                localStorage.removeItem('myFavorites');
                localStorage.removeItem('auraflixSeriesMemory');
                localStorage.removeItem('auraflixUnlockedMemory'); // También borramos la memoria de desbloqueos
