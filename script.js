// =========================================================
// 🔥 AQUÍ EMPIEZAN LOS CAMBIOS PRINCIPALES Y LA ANIMACIÓN
// =========================================================
function openModal(id, typeHint, fromDeepLink = false) {
    ...
    // ==========================================================
    // ANIMACIÓN HERO CORREGIDA (650ms de pura fluidez)
    // ==========================================================
    ...
    if (clickedCard && !fromDeepLink) {
        ...
        clone.style.transition = 'all 0.65s cubic-bezier(0.25, 1, 0.5, 1)';
...
// ==========================================================
// NUEVA FUNCIÓN: ENLACES SECUENCIALES (linkvip.js)
// ==========================================================
window.executeUnlock = function() {
    if (currentModalItem) {
        ...
        // 1. Determinar el TicketKey correcto
        ...
        if (shortLink) {
            ...
            window.location.assign(shortLink); // Navegación directa
        } else {
...
function setPlayerVideo(url, overlayText = null, isBlocked = false) {
    ...
    if (overlayText && !isBlocked && isDirectFile) {
...
function closeModalInternal() {
    ...
    modal.classList.remove('active', 'open');
    modalContent.style.transition = 'transform 0.65s cubic-bezier(0.25, 1, 0.5, 1), opacity 0.65s ease';
    modalContent.style.transform = 'scale(0.9) translateY(40px)';
    ...
    if (btnInstall) {
        btnInstall.onclick = () => {
            if(typeof AURA_LINKS !== 'undefined' && AURA_LINKS.appDownload) {
                window.location.href = AURA_LINKS.appDownload;
            } else {
...
window.addEventListener('popstate', (event) => {
    ...
    // Abrimos el modal directo sin animación compleja
    modal.style.display = 'flex';
    playerWrapper.style.opacity = '1';
...
// Cierre directo sin transición pesada
function closeModalInternal() {
    const modal = document.getElementById('videoModal');
    modal.classList.remove('active', 'open');
