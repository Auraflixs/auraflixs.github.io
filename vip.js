...
window.VIP_SYSTEM_STATUS = "off";    // "on" = Sistema VIP activado | "off" = Oculto
window.UNLOCK_LINK_STATUS = "on";    // "on" = Botón de acortador activado | "off" = Oculto
...
document.addEventListener('DOMContentLoaded', () => {
    // Si AMBOS sistemas están apagados, ponemos estado libre en el perfil
    ...
    if (btnActivate) {
        btnActivate.onclick = () => {
            ...
            if (inputClean === correctClean) {
                activateVipMode(12); // Activa por 12 horas
                document.getElementById('vipModal').style.display = 'none';
                input.value = '';
                // Asumiendo que showToast está en script.js, lo usamos; si no, alert
                if (typeof showToast === 'function') {
                    showToast("¡Felicidades! Ahora eres VIP por 12 horas.");
                } else {
                    alert("¡Felicidades! Ahora eres VIP por 12 horas.");
                }
            } else {
                if (typeof showToast === 'function') {
                    showToast("Código incorrecto para el día de hoy.");
                } else {
...
// Modificado para aceptar horas (para ser compatible con codevip.js también)
window.activateVipMode = function(hours) {
...
window.isUserVip = function() {
    // Si AMBOS sistemas están apagados, TODO ES GRATIS.
    if (window.VIP_SYSTEM_STATUS === 'off' && window.UNLOCK_LINK_STATUS === 'off') {
        return true; 
    }

    // Si al menos uno está activado, comprobamos si el usuario es realmente VIP
    const expiry = localStorage.getItem('auraflixVipExpiry');
    if (!expiry) return false;
    
    const now = new Date().getTime();
    if (now < parseInt(expiry)) {
        return true; // El usuario aún tiene tiempo VIP válido
    } else {
        localStorage.removeItem('auraflixVipExpiry'); 
        return false; // El VIP caducó
    }
};

window.checkVipTimerUI = function() {
    // Si todo es gratis, no mostramos cronómetro ni botón
    if (window.VIP_SYSTEM_STATUS === 'off' && window.UNLOCK_LINK_STATUS === 'off') return;
    ...
    // Si el administrador apaga el VIP visualmente, ocultamos el botón de "Ser VIP"
    if (window.VIP_SYSTEM_STATUS === 'off' && btnBecome) {
        btnBecome.style.display = 'none';
    }

    if (isVip) {
        // Ocultar botón de ser VIP si ya lo es
        if (btnBecome && window.VIP_SYSTEM_STATUS === 'on') btnBecome.style.display = 'none'; 
        
        // Mostrar temporizador solo si el sistema VIP está "on"
        if (timerContainer && window.VIP_SYSTEM_STATUS === 'on') timerContainer.classList.remove('hidden'); 
        
        if (badge && window.VIP_SYSTEM_STATUS === 'on') {
            badge.innerText = "Usuario VIP";
            badge.classList.add('vip-active');
        }
        
        if (window.VIP_SYSTEM_STATUS === 'on') startTimerInterval(); 
    } else {
        // Mostrar botón de ser VIP solo si el sistema está "on"
        if (btnBecome && window.VIP_SYSTEM_STATUS === 'on') btnBecome.style.display = 'inline-block'; 
