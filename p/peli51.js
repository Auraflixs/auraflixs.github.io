window.moviesList = window.moviesList || [];

window.moviesList.push({
    id: "p51",
    title: "Los Ilusionistas 3",
    tags: ["The Illusionists 3", "Now You See Me 3", "Ahora me ves 3"],
    image: "https://raw.githubusercontent.com/auraflixx/afx/refs/heads/main/app/Aspr0HcItNVbgLC8cV0ZAO0EBcc.webp",
    trailer: "https://www.youtube.com/embed/-E3lMRx7HRQ",
    video: "https://streamtape.com/e/K7mLzM3Oo4HzaW/Los_ilusionistas_3_Ahora_me_ves%2C_ahora_no_DUAL_1080p_id_425274.mkv.mp4",
    year: 2025,
    genres: ["Suspenso", "Crimen", "Misterio"],
    info: "Los Cuatro Jinetes regresan para una nueva aventura que promete elevar el arte de la magia a niveles nunca antes vistos. Con el regreso del elenco original y la incorporación de nuevos talentos, el equipo deberá enfrentarse a un adversario que conoce todos sus trucos. En un mundo de tecnología avanzada, demostrarán que el ojo humano siempre puede ser engañado.",
    featured: true
});
